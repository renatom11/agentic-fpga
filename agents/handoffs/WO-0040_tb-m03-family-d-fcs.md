...
- `test/xgmii_rx_64/test_m03_d.ml` — new file, 445 lines: `run_d1`, the `d3_ordering` type and `run_mixed_pair`/`assert_frame`/`split_at_first_tlast` machinery shared by D2's D3-partner check and D3, `run_d2_d1_partner`/`run_d2_d3_good_member`, `run_d3`, **three** `%expect_test`s (D1, D2, D3), plus D4's trailing declaration comment — no test function for D4 [count corrected per RV-0040-R4; the original read "four"].
...
- **Whether `dune runtest` is green, and whether the three `%expect_test`s pass against real `Xgmii_rx_64`** [count corrected per RV-0040-R4].** Predicted PASS (§5), never confirmed.
