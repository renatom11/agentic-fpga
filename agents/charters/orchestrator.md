...
- **Mutation-window operator** (PROTOCOL §10): for each module's campaign, apply the auditor's manifests from `docs/reports/audit/mutations/` **transiently in an uncommitted working tree**, spawn/run dv_lead's suite against the mutated tree, revert fully before any staging, and never let a manifest patch reach a commit. Never spawn an RTL-line or worker agent while a manifest is applied. Sequence campaigns after rtl_lead's `RV-` ACCEPT and before dv_lead's `SO-` PASS.
...
| auditor | Audit reports; **mutation manifests for transient application**; relay-fidelity spot-check results; DV-escape ledger entries | Spawn + audit-cycle `WO-` packets; commit ranges to audit; clean mutation-window operation (apply, run, revert); **verbatim** onward relay of its findings (E4 → sponsor) |
