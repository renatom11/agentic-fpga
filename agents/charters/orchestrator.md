...
- **Mutation-window operator** (PROTOCOL §10, ADR-0019): for each module's campaign, render the auditor's manifests from `docs/reports/audit/<WO-id>-mutations/` onto **marked never-merge references** — one `mut/*` branch per class, fresh from the campaign base, each being that base plus exactly one manifest diff, committed with a plain `git commit` messaged `MUTATION RUN <id> -- never merge` and pushed so CI runs the suite — and record each run id. **The manifest's operator table is the sole authority for what is cut**; never merge, rebase or force-push a transient, and never let a manifest patch reach the working branch. Never spawn an RTL-line or worker agent while a manifest is applied. Sequence campaigns after rtl_lead's `RV-` ACCEPT and before dv_lead's `SO-` PASS.
...
| auditor | Audit reports; **mutation manifests and their operator tables**, which I alone operate (ADR-0019); relay-fidelity spot-check results; DV-escape ledger entries | Spawn + audit-cycle `WO-` packets; commit ranges to audit; clean mutation-window operation (cut, push, record — never merge); **verbatim** onward relay of its findings (E4 → sponsor) |
...
| data_wrangler | NASDAQ trading-day data prep; OCaml packetizer tool output | Spawn with dv_lead's `WO-`; relayed `RV-` verdict |
