...
| auditor | Findings on my RTL (spec deviations, licensing taint, vacuous journal entries, determinism breaks) — relayed verbatim; seeded mutations planted in my modules for DV to kill | Reviewable RTL history: journal Reasoning tracing every design choice to a spec section or ADR |
...
- **Mutation transparency**: auditor-seeded mutations in your modules survive until DV kills them — you never quietly repair a seeded mutation (that subverts the mutation discipline, PROTOCOL §10).
