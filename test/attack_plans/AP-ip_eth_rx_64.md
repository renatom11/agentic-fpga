...
| Row | Attacks | Stimulus | Observable | Kills | Status |
|---|---|---|---|---|---|
| **M14-F1** | REQ-016, §7's per-octet constant | The directed set of M14-D1 through an idle-injection wrapper at **0, 1 and 7** cycles between payload words, **after** the header | The output word sequence is unchanged; the **per-octet constant L = 12** is unchanged for every octet; every octet is delayed by exactly 8 octet times per injected cycle; `ip_hdr_valid` still leads payload word 0 by one cycle | A design that treats an idle cycle as a word (advancing the word index) — it mis-assembles the realignment and delivers garbage from the next datagram's position | ASSERT |
| **M14-F2** | REQ-611, §7's scoping, **C-27** | The same wrapper with the idle cycles landing **inside the header** — between input words 0 and 2 — at 1 and 7 cycles | The parse latency is **3 + k**, exactly the number of injected cycles, and that grown value is **asserted**; L = 12 is unchanged | Two things at once. (i) A design whose header parse is cycle-counted rather than word-counted: it reports the record early and mis-decodes the destination. (ii) **The bench itself** — a REQ-611 + REQ-016 bench asserting 3 under injection fails a conformant M14, which is exactly what C-27 found; this row turns C-27's scoping from a silence into a positive assertion | ASSERT |
| **M14-F3** | REQ-016, §6.1's gapped paragraph | — | **§6.1's cycle formulas (`Ci + m`, `Ci + 3`, `Ci + 4 + j`) are NOT asserted under injection.** They are stated on a gapless stimulus; the gap-invariant quantity is L = 12 and that is what M14-F1 asserts | — | NO-ASSERT |
