...
| **M03-D3** | REQ-104, §6.1's seeding rule, §6.1's drain paragraph | **CORRECTED 2026-08-06 (`J-dv_lead-0038`) — the original ordering did not kill the design this row names.** Two 64-octet frames at the **minimum** 12-octet gap (start-to-start 10 or 11 cycles, §0.3), in **both orderings**, at both start lanes — four schedules. **Pair A, good-FCS then bad-FCS.** **Pair B, bad-FCS then good-FCS** | Pair A: frame 1 `tuser`[0] = 0 and **no strobe**; frame 2 `tuser`[0] = 1 with exactly one `error_bad_fcs` on **its own** `tlast` cycle. Pair B: frame 1 `tuser`[0] = 1 with one `error_bad_fcs` on its `tlast` cycle; frame 2 `tuser`[0] = 0 and no strobe | **Pair A** kills a design that reads the CRC register at the `tlast` cycle rather than carrying the verdict with the frame. The arithmetic: at a lane-0 start the first frame's terminate character is at octet time 80 (cycle 10, lane 0), its `tlast` word is at cycle 11, and `ifg` = 12 from the terminate inclusive puts the second frame's start character at octet time 92 — **cycle 11, the same cycle** — so §6.1's `Preamble` seeding has already re-seeded the register. The wrong design then reads the fresh seed, which differs from REQ-304's residue, and reports **bad**. That only discriminates where the first frame is **good**: with a bad first frame both designs say "bad" and the row passes vacuously, which is what the original stimulus ordering did. **Pair B** is retained because it kills a *different* design — one that latches the abort bit and fails to clear it between frames | ASSERT |
