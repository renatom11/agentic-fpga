...
#### 4.I-R THE TWO CELLS THAT WERE MINE TO EDIT, EDITED — the `-0024`-era carry-forward discharged (2026-08-11, `J-dv_lead-0169`)

> **DATED ANNOTATION. The M03-I4 and M03-I5 cells above are 2026-08-07's and
> 2026-08-04's and are left standing**; this block sits beside them and each cell
> carries a pointer to it. **No row is added, converted or re-statused; the census
> is unmoved at 78 rows / 62 ASSERT / 7 NO-ASSERT / 4 NO-STIMULUS / 4 STRUCTURAL /
> 1 GAP**, re-measured by a status-cell pass over every row table at this commit
> and not carried forward.

**The carry-forward, and why it sat.** `J-architect_docs_lead-0024` closed with
*"`AP-xgmii_rx_64.md` §4.I's M03-I4 and M03-I5 cells are dv_lead's to edit and I
have not touched them; the ruling they were waiting on is this entry."* The
architect's own ledger has carried it as row 30 ever since — *"dv's next plan
round"* — and `FINDING SO-1-A`'s §6 repair joined it there on the ground that
they share a file and a carrier. **This is that round, and both are paid in this
commit.**

**What was actually stale, stated exactly, because it is less than the
carry-forward's wording implies and the difference is worth recording.** The
substantive halves of both cells were repaired at the time and are still right:
M03-I4's Observable already carries the **re-ruled `D(m)`** (`1f3c04c`,
`J-architect_docs_lead-0025`, countersigned `J-dv_lead-0086`), already withdraws
§6.1 item 3's lane-0 carve-out, and already marks every latency class **reported,
never asserted**; M03-I5's Observable already states that the gap-invariant
quantity is the per-output-event delay and not the per-octet constant. **Two
things were stale and both were metadata rather than claims**:

| # | What | Repair |
|---|---|---|
| 1 | M03-I4's Observable called `FINDING F-1` *"outstanding with architect_docs_lead"* | **Status corrected in the cell**: F-1 is **DISCHARGED** — the repair landed in **SPEC-M03 §6.1 item 2** at **`d54c931`** (`J-architect_docs_lead-0026`), recorded at requirements.md §13's 2026-08-04 row under `J-architect_docs_lead-0030`, which verified it term by term against F-1's own text rather than accepting it |
| 2 | M03-I5's `Attacks` cell cited requirements.md §0.5 at **`a77017c`** only, while its sibling M03-I4 has cited *"`a77017c`, re-ruled at `1f3c04c`"* since `J-dv_lead-0086` | **Citation completed in the cell.** A reader arriving at M03-I5 first would have gone to the superseded text and found a `D(m)` the programme replaced four months of rounds ago |

**What this repair deliberately does NOT do.** It does **not** re-measure or
re-assert M03-I4's three-class latency table: §0.1 binds its own author, a status
correction is not a measurement, and re-affirming a set claim I did not re-take
would be the exact move that rule exists to stop. It does **not** re-open
`FINDING F-1`'s substance — the architect's discharge was verified at its own
site and this block cites it rather than re-deriving it. It does **not** touch
`BUG-0002`, whose packet reads **CLOSED with a stated carve-out** and whose
remaining ledger item (the architect's row 32, *"`BUG-0002` cannot close on the
`-0025` ruling"*, owner dv_lead) has its own carrier — **a bug round, which this
is not** — and is left carried rather than quietly absorbed. And it does **not**
move `M03-I4`'s or `M03-I6`'s discharge state, which the standing census above
already fixes.

**One observation the repair produced, recorded because it is the reason the debt
was cheap to carry and expensive to notice.** Both stale items are *pointers* —
a status word and a SHA — and neither would ever fail a test. **A claim that can
only be falsified by a reader is the kind that survives a green suite
indefinitely**, which is why the carry-forward list, and not the bench, is the
instrument that found them; and it found them because a *seat other than mine*
kept writing the row down every round.
...
| REQ-007 | M03-D1, M03-E1, M03-F1, M03-G1, M03-H1 (set); M03-E3, M03-F2, M03-B2, M03-B4 (the zero-delivered cases where the bit has no word — §0.7). **+ M03-M9 — HOMED 2026-08-11 (`J-dv_lead-0169`), `FINDING SO-1-A`(b)**: a `STRUCTURAL` no-instance row — M03 is the *origin* of `tuser`[0] and inherits no abort, so REQ-007's inheritance clause has nothing to reach here. It costs no coverage and was reachable from this map by neither name nor range |
...
| REQ-011 | M03-C1, M03-C3, M03-C4; standing obligation 1. **+ M03-C5 — HOMED 2026-08-11 (`J-dv_lead-0169`), `FINDING SO-1-A`(a)**: the `BUG-0001` / prediction `P-1` probe at 1513 and 1516 octets, `test/xgmii_rx_64/test_m03_c.ml:408`, **a landed green `ASSERT` unit** this map reached by neither name nor range. `WO-0079` §3.B homed it at this REQ by delivery on 2026-08-11; this line is the plan statement that delivery had to do without |
| REQ-012 | M03-A5 |
...
| REQ-101 | M03-A1, M03-A2, M03-A3, M03-A4. **+ M03-O4 — HOMED 2026-08-11 (`J-dv_lead-0169`), `FINDING SO-1-A`(b)**: a `NO-STIMULUS` row — a start character outside lanes 0 and 4 is never driven (§3, REQ-018's contract), so REQ-101 has no instance to assert there. It costs no coverage and was reachable from this map by neither name nor range |
...
| REQ-103 | M03-C1, M03-C3, M03-E1, M03-F1, M03-G1, M03-H1. **+ M03-C5 — HOMED 2026-08-11 (`J-dv_lead-0169`), `FINDING SO-1-A`(a)**: `test/xgmii_rx_64/test_m03_c.ml:408`, landed and green. **Its omission understated this cell**, which is what made the finding MINOR-with-cost rather than clerical |
| REQ-104 | M03-D1, M03-D2, M03-D3, M03-D4, M03-M2, M03-M3, M03-M4, **M03-M10** (the sub-5 class, where REQ-104 supplies neither operand) |
| REQ-105 | M03-B2, M03-E1, M03-E2, M03-E3, M03-E4, M03-G4 (second epoch), **M03-G8** (first epoch), M03-M5, M03-M7, M03-N1. **+ M03-E5 — HOMED 2026-08-11 (`J-dv_lead-0169`), `FINDING SO-1-A`(a)**: the `/E/` at preamble positions 1..7, `test/xgmii_rx_64/test_m03_e.ml:799`, **a landed green `ASSERT` unit** this map reached by neither name nor range, and whose omission understated this cell |
| REQ-106 | M03-C1, M03-C2 |
| REQ-107 | M03-B3, M03-F1 … M03-F5, M03-M1, **M03-M10**. **+ M03-M8 — HOMED 2026-08-11 (`J-dv_lead-0169`), `FINDING SO-1-A`(b)**: a `NO-STIMULUS` row (§9 ruling 8 — `error_runt` and `error_oversize` have disjoint octet ranges, so the co-occurrence has no stimulus). It costs no coverage and was reachable from this map by neither name nor range |
| REQ-108 | M03-G1 … M03-G8, M03-M2, M03-M6, M03-M7. **Read the two epochs**: G1, G3 and G4 drive REQ-108's window after the frame's own terminate; **G7 and G8 drive it before**; G6's frame has no terminate so its whole window is the first epoch. **+ M03-M8 — HOMED 2026-08-11 (`J-dv_lead-0169`), `FINDING SO-1-A`(b)**: the same `NO-STIMULUS` row, homed at both requirements whose co-occurrence it declares to have no instance |
...
### 6.1 THE CONVERSE RULE — rows homed by no REQ line, and the repair of `FINDING SO-1-A` (2026-08-11, `J-dv_lead-0169`)

> **DATED ANNOTATION. The table above is 2026-08-02's, grown by appending, and
> this block does not rewrite a cell of it.** The seven cells that gained a
> homing this round carry their addition **after** their original text with a
> dated marker, so the list a reader of `WO-0079` or of `SO-xgmii_rx_64.md`
> §2.8-R was quoting is still readable in place. **No row is added, converted or
> re-statused; no discharge count, no bar and no anchor moves.**

**The defect this repairs, in its own words** (`FINDING SO-1-A`, MINOR, mine,
minted at `WO-0079` §6.1 and carried at `SO-xgmii_rx_64.md` §3.9 with the
carrier *"the next round opening `AP-M03`"* — which this round is). §6's opening
rule reads *"A REQ with no behavioural row carries the reason."* **It says
nothing about a row with no REQ line, and six rows fell through that hole.**

**MEASURED AT THIS TREE, before and after** — the polarity dimension, because the
claim is that a naming **does not exist** (§0.1's rule, and `FINDING
RV-0078-S2-13`'s). The command is the finding's, **with one change forced by the
act of quoting it, and the change is itself a finding** — see the note below the
block:
...
PY
78 72 ['M03-C5', 'M03-E5', 'M03-M8', 'M03-M9', 'M03-O4', 'M03-O5']      (before)
78 78 []                                                               (after)
```

> **`FINDING AP-6-1` (MINOR, mine, against my own first draft of this block, and
> caught by running it) — QUOTING A TEXT-MATCHING COMMAND INSIDE THE DOCUMENT IT
> MATCHES OVER CHANGES THAT DOCUMENT'S MATCH SET.** `FINDING SO-1-A`'s script
> bounds its slice with `txt.index('## 7. Machinery this plan requires')` — a
> **substring** search, not a line-anchored one. The moment that literal is
> written into this block, **the first occurrence of it in the file is the
> quotation**, which sits *before* the real §7 heading; the slice then ends inside
> the code fence, the homing table below is excluded, and the check reports
> `['M03-O5']` — **a false unhomed row produced by the act of documenting the
> repair.** I wrote that draft, ran the command against the file it was in, and
> got the wrong answer from the right script. The repair is the two lines above:
> the boundaries are found by a **line-anchored** header match (`(?m)^## (\d)\. `),
> which a quotation inside a fence cannot satisfy, so the command is
> quotation-stable and can live in the document it grades. **The general form,
> banked**: *a checker whose subject can contain the checker's own text needs a
> match rule that its own quotation cannot satisfy.* This is the same family as
> the standing rule that an instrument grading documents by a rule may not itself
> violate that rule — met here at the one place it could not be avoided, since
> §0.1 requires the command to be quoted **with** its measurement.
>
> **AND IT HAPPENED A SECOND TIME IN THE SAME BLOCK, through the other instrument
> — `FINDING AP-6-2` (MINOR, mine, same round, same cause).** The disposition
> table below was first written with its first column in the row tables' own
> notation, `| **M03-C5** | …`, which is **exactly the pattern the row census
> matches**. The raw count went from **78 to 84** — six annotation lines joining
> the inventory of declared rows — while the *set*-based derivation stayed at 78
> only because every id in the table was already a declared row, i.e. the defect
> was masked by an accident of content. The repair is notational: the table's
> first column now carries its ids in backticks, which no census pattern matches,
> and the raw count is **78** again. **The general form, and it is the sharper of
> the two**: *a document that is measured by a pattern must not write its
> annotations in the notation that pattern selects* — an inventory taken from how
> members are **named** is a different set from the one the rule selects, and here
> the naming was mine and the rule was mine and they were one paragraph apart.
> **`AP-M04` §6.1 carried the identical defect at the identical table and was
> repaired in the same commit** (81 raw against 80 declared), which is what makes
> this a class rather than a slip.

**The `78` is the same 78 the standing census counts, re-measured here and not
carried forward**: a status-cell pass over every row table at this commit gives
**78 rows / 62 ASSERT / 7 NO-ASSERT / 4 NO-STIMULUS / 4 STRUCTURAL / 1 GAP**,
unchanged by this round in every cell.

**Where each of the six is now homed, and what each cost.**

| Row | Status | Homed at | What its absence cost |
|---|---|---|---|
| `M03-C5` | ASSERT, landed green (`test_m03_c.ml:408`) | **REQ-103** and **REQ-011** | **Coverage.** Had `WO-0079` transcribed §6 mechanically, REQ-103 and REQ-011 would have omitted landed evidence — *the failure mode the matrix exists to prevent, arriving through the instrument meant to prevent it* |
| `M03-E5` | ASSERT, landed green (`test_m03_e.ml:799`) | **REQ-105** | The same, at REQ-105 |
| `M03-M8` | NO-STIMULUS (§9 ruling 8) | **REQ-107** and **REQ-108** | No coverage. A defect in the map's rule, not in the bench |
| `M03-M9` | STRUCTURAL (REQ-007) | **REQ-007** | No coverage; same |
| `M03-O4` | NO-STIMULUS (REQ-101) | **REQ-101** | No coverage; same |
| `M03-O5` | NO-ASSERT | **here — no REQ line, permanently** | No coverage. It is a **plan-wide prohibition row**: it constrains what this plan's benches may assert and attacks no requirement, so no REQ line can reach it and the map needed a place that was not a REQ. This block is that place |

**THE RULE, stated as the converse of §6's own and binding this map from here on:**

> **Every declared row appears in §6's table or in §6.1, and nowhere else is
> needed.** A REQ with no behavioural row carries the reason; **a row with no REQ
> line carries its home.** A row that is `NO-STIMULUS`, `STRUCTURAL` or
> `NO-ASSERT` is still a declared row and is still homed — its status says what a
> bench does with it, never whether the map must reach it.

**What this repair does NOT do, each omission deliberate.** It does **not**
convert `M03-C5` or `M03-E5` into new coverage: both were landed and green before
this round and both were already cited in `docs/specs/traceability.md` by
`WO-0079`'s delivery — **what changes is that the plan now says what the matrix
already said**, which is the direction of authority the matrix depends on. It
does **not** re-open `SO-xgmii_rx_64.md`: that packet's `PASS` at `41fead6` is
unaffected, and §3.9's row for this finding is discharged by this block rather
than amended in place. It does **not** touch `WO-0079` §3's cells. And it does
**not** claim the map is now complete in any other sense — the check it passes is
exactly the one the script above runs, over row ids, and no other.

**The rule is carried forward rather than only applied backward.** `AP-M04`
(`test/attack_plans/AP-xgmii_tx_64.md`, landed in this same commit) has the
converse rule in its §6 from its first line, and **its own draft failed the check
on five rows** before it was committed — the instrument's first act at the new
plan was to convict its author, which is the strongest evidence available that
the rule is worth carrying and not merely worth stating.
...
| 2026-08-11 | **`FINDING SO-1-A` DISCHARGED AT ITS NAMED CARRIER, and the `-0024`-era §4.I carry-forward with it — the two debts that shared this file and this carrier, paid in one round** (`SO-xgmii_rx_64.md` §3.9's row, whose carrier reads *"the next round opening `AP-M03`"*; `J-architect_docs_lead-0024`'s closing bullet and the architect's ledger row 30, which has carried the §4.I cells as *"dv's next plan round"* since 2026-08-04). **NO ROW ADDED, NO ROW CONVERTED, NO STATUS MOVED, NO DISCHARGE COUNT AND NO BAR MOVED: 78 rows, 62 ASSERT, 7 NO-ASSERT, 4 NO-STIMULUS, 4 STRUCTURAL, 1 GAP** — re-measured at this tree by a status-cell pass over every row table, before and after the edits, and **not** carried forward. **(1) §6 GAINS ITS CONVERSE RULE AND SIX HOMINGS.** Seven cells gain an appended, dated homing — `M03-C5` at REQ-103 and REQ-011, `M03-E5` at REQ-105, `M03-M8` at REQ-107 and REQ-108, `M03-M9` at REQ-007, `M03-O4` at REQ-101 — and the new **§6.1** homes `M03-O5`, which no REQ line can reach because it constrains the plan rather than the design. **Each original cell's text is left standing and the addition follows it**, so the list `WO-0079` and `SO-` §2.8-R quoted is still readable in place. The finding's own measurement is re-run at this tree: **78 / 72 / six unhomed** before, **78 / 78 / none** after. **(2) §4.I-R PAYS THE TWO CELLS, AND THEY WERE STALER IN METADATA THAN IN SUBSTANCE.** `M03-I4`'s Observable called `FINDING F-1` *"outstanding"* when it was discharged at `d54c931` (`J-architect_docs_lead-0026`, recorded `J-architect_docs_lead-0030`), and `M03-I5`'s `Attacks` cell cited §0.5 at `a77017c` alone where its sibling has cited the `1f3c04c` re-ruling since `J-dv_lead-0086`. Both cells' substantive claims were already right and **are not re-measured or re-asserted** — a status repair is not a licence to re-affirm a set claim it did not take (§0.1, applied to its own author). `BUG-0002` is untouched and its ledger row keeps its own carrier, a bug round. **(3) TWO FINDINGS AGAINST THIS ROUND'S OWN INSTRUMENTS, both caught by running them.** **`AP-6-1`**: quoting a substring-bounded slice command inside the document it measures makes the quotation the first match — the check reported a **false** unhomed row, and the repair is a line-anchored boundary that a fenced quotation cannot satisfy. **`AP-6-2`**: writing an annotation table in the row tables' own bold notation put six annotation lines into the row census (78 → 84 raw), masked by the set derivation because every id was already declared; repaired by backticking the column, and the identical defect was found and repaired at `AP-M04` §6.1 in the same commit. **(4) The rule is carried forward, not only applied backward**: `test/attack_plans/AP-xgmii_tx_64.md` lands in this same commit with the converse rule in its §6 from its first line, and its own draft failed the check on five rows before commit. | dv_lead, `J-dv_lead-0169` |
