...
4. **No `SO-xgmii_tx_64.md` may report REQ-206 coverage while `M04-G10` is
   ...
   **PERMITTED FORM (i) IS SATISFIED — recorded 2026-08-12 at the absorption of
   `WO-0082`, `J-dv_lead-0192`, which is the only place `WO-0082` §9.8 permits it
   to be recorded.** `M04-G10` is **measured**, on the machinery §7 item `T-7`
   commissions, at four members — shapes (a) `W = 1` (`B ∈ {1, 8}`) and (b)
   `W = 2` fully pre-loaded (`B ∈ {9, 16}`) — with the **route's own precondition
   asserted first and as a whole list** (nine acceptances `[C … C+8]` at (a), ten
   `[C … C+8; C+11]` at (b) with the hole at `C+9`/`C+10`), so the silence that
   follows is a silence **on the route** and not on some other stimulus that
   happens to be quiet. `BUG-0004`'s routes 2 and 3 stop being *fixed by
   derivation and never measured in either design* at `65ba148`.
   **What this does NOT do, stated in the same breath because this item is a
   sign-off bar and will be read by a sign-off round**: it does **not** make
   REQ-206 covered — `M04-G1` … `G8` are all outstanding and this bar was never
   the only thing between this module and a REQ-206 claim; it does **not**
   discharge `M04-G4`, the highest-value row in the family; it lifts **no** other
   bar; and **form (ii) remains available and unused**. **The bar is satisfied,
   not lifted**: a later `SO-` still states which form it rests on, and this note
   is the artefact it cites for form (i).
...
| **M04-F6** | REQ-204, REQ-206, §9's stream-effect cell (*"the gap is then served from that terminate character"*) | An underflowed frame (family G's stimulus) followed by a normal frame, `cfg_ifg` = 12 | The gap is measured from §9's terminate character — **lane 1** of the `/E/` word, so `t = 1` — giving `g = ⌈13/8⌉ = 2` words and a gap of **15** octets, with the next start character in lane 0 | A design serving **no** gap after an abort (the next preamble immediately); and a design placing the next start character in the **wrong word** after an abort — a recorded gap of **7** or **23** rather than 15 — which this row's *exact* `= 15` assertion (never `≥ cfg_ifg`) kills where a REQ-204-inequality-only check would pass it. ~~Struck 2026-08-12 (`FINDING WO-0084-S1`, MINOR, mine, `J-dv_lead-0196`): *"a design measuring the gap from the `/E/` character rather than from the `/T/` (`t = 0`, gap 16). The second defect is one octet from conformant and passes every ≥ cfg_ifg check"*.~~ **The `16` was arithmetically unreachable and was so when written**: `Tx_decoder.gaps` counts from the terminate character on the wire — §9 fixes it as the `/T/` in **lane 1** — and REQ-201 fixes every start character in lane 0, so every gap this decoder can record after an abort is `8k − 1` octets, i.e. **`≡ 7 (mod 8)`: {7, 15, 23, …}, and 16 is not in the set**. The `/E/`-vs-`/T/` design (the sealed **IC-10**, ABORT-GAP-FROM-`/E/`) is **not** one octet from conformant and is **not visible at all** at this row's `cfg_ifg = 12`: `⌈(12 + 0)/8⌉ = ⌈(12 + 1)/8⌉ = 2`, so both readings place the next start character in the **same word** and the decoder records **15 either way** — which is exactly why this row, pinned at 12, cannot see IC-10 and why its campaign sibling (`IC-10`, un-rendered) was sealed to survive. **The class separates only once `cfg_ifg` varies** — at `cfg_ifg ≡ 0 (mod 8)` the two readings differ by a **whole word** (at 16, conformant 23 against the defect's 15) — and its coverage is **re-homed to `M04-F7`**, not dropped. The identical `16` in `test/xgmii_tx_64/test_m04_f.ml`'s `M04-F6` comment and failure-message is the finding's **second site** (seal §6); it is corrected when a round next opens that bench and is folded into the `M04-F7` carrier WO. **SECOND SITE CLOSED 2026-08-22 at `a0cf4dd`** (`WO-0085`, tb_writer `J-tb_writer-0047`, reviewed at `J-dv_lead-0199`): the three sites carrying the figure are corrected — the file's own `M04-F6` addendum docstring, the comment above assertion 3, and assertion 3's failure-message string — and the `= 15` assertion is byte-unchanged, so this row's Observable and Kills are untouched by the correction. **A FOURTH SITE was missed and is corrected at review**: the `U26` banner comment (*"the one octet that separates conformant from not"*) **paraphrased the struck claim without quoting its figure**, so a site list enumerated on the string `16` could not reach it — `FINDING WO-0085-R1` (MINOR, mine, `J-dv_lead-0199`), the general form being that a correction scoped by the expression of a claim misses every restatement of it | ASSERT |
| **M04-F7** | REQ-204, REQ-206, REQ-802, §9's stream-effect cell, §0.3's convention, §4's identity, §4.3's `ifg` row | An underflowed frame (family G's stimulus — §9's terminate character is the `/T/` at **lane 1**, `t = 1`) followed by a normal frame, swept over `cfg_ifg ∈ {12, 16, 24}`, each value stable from well before the abort word | For each member `Tx_decoder.gaps` has **exactly one** abort-gap entry — its list length asserted first, so a two-gap run cannot pass a one-gap check — of `g = ⌈(cfg_ifg + 1)/8⌉` words and `8g − 1` octets: **15, 23, 31**; the next start character is in lane 0; every conformant abort gap is `≡ 7 (mod 8)` | The **IC-10 / ABORT-GAP-FROM-`/E/`** design — the campaign's un-rendered predicted survivor, and the abort sibling of class-11's normal-gap ignore: measuring the gap from the `/E/` in lane 0 (`t = 0`) rather than the `/T/` in lane 1 places the next start character `g = ⌈cfg_ifg/8⌉` words after the abort word, which the decoder records as `8g − 1` = **15, 15, 23** — **identical to conformant at `cfg_ifg = 12`** (why M04-F6, pinned at 12, cannot see it) and a **whole word short at `cfg_ifg ∈ {16, 24}`** (15 against 23, 23 against 31); and a design ignoring `cfg_ifg` on the abort path (a fixed abort gap), which the default 12 hides. **The discriminator is the word count, not one octet: `cfg_ifg = 12` distinguishes neither IC-10 nor class-11, and 16 is the smallest member that separates both — the axis is the instrument, not any single value** | ASSERT |
...
12. **Three rejections carried by `M04-F7`, recorded 2026-08-12 with the row**
    (`WO-0084-S2`, mine — this list is the auditor's mining ground and a row
    added later owes its rejections just as much as one added at the first
    commit). **(a) Asserting the `/E/`-vs-`/T/` (IC-10) separation at
    `cfg_ifg = 12`.** Rejected: at 12 the two readings are byte-identical on the
    wire (both record gap 15), so an assertion of a difference there is vacuous
    and green against every design — the separation exists only at
    `cfg_ifg ≡ 0 (mod 8)`, which is why `M04-F7`'s discriminating members are 16
    and 24 and 12 is carried only as the control that proves the invisibility.
    **(b) A full `cfg_ifg` sweep 12 … 255 on the abort path.** Rejected: the only
    residue class that changes the abort gap's *word count* is `cfg_ifg mod 8`
    (separation at `≡ 0`), and the 8-bit-accumulator saturation is `M04-F3`'s
    member 255 on the normal path — 255 ≡ 7 (mod 8) does **not** separate `/E/`
    from `/T/` (both give `g = 32`), so it buys nothing on the abort path; {12,
    16, 24} names the control and two separating members, which is the
    derivation's complete set. **(c) Widening `M04-F6` to carry the axis instead
    of adding `M04-F7`.** Rejected: `M04-F6` is pinned at `cfg_ifg = 12` to make
    its *exact* `= 15` assertion against the word-count (branch γ) defect, and
    that frozen value is load-bearing; the configuration axis is a distinct
    instrument and takes its own row so `M04-F6`'s assertion is not disturbed and
    the two rows' kills stay separable.
...
| REQ-204 | M04-F1 … M04-F7; M04-I4; M04-K3, M04-K4 (the sampling rules); M04-L5 (no instance for the first frame after `clear`) |
| REQ-205 | M04-E1 … M04-E5 |
| REQ-206 | M04-G1 … M04-G10; **M04-E5** (REQ-205 has no instance on an underflowed frame); M04-F6 (the gap after one), M04-F7 (that abort gap tracking `cfg_ifg`, the IC-10 sibling of class-11); M04-L3 (the co-occurrence with REQ-009). **`M04-G9` and `M04-G10` were added 2026-08-11** — the two shapes at which REQ-206's window is **empty** and the strobe must stay silent — and **§0.2 item 4 bars an `SO-` from reporting this REQ's coverage while `M04-G10` is neither measured nor declared a gap** |
...
| REQ-802 | M04-K1, M04-K3, M04-K4; M04-F3, M04-F7; **M04-O4** (the prohibited `cfg_ifg` range) |
...
them and not only for the families it was written for. **Re-measured again
2026-08-12, after `M04-F7` landed** (`WO-0084-S2`): the figure moved to `83 83 []`
and is quoted at its new value with the same command, `M04-F7` being reached
through §6's REQ-204 range `M04-F1 … M04-F7` by the same range-expansion loop.
**Re-measured again 2026-08-22, at the `WO-0085` absorption** (`J-dv_lead-0199`):
the figure is **unchanged at `83 83 []`** — this round adds **no row** and
discharges two that were already declared and already homed — and it is re-run
rather than carried forward **because that is the point of §0.1(i)**: a census
whose expected value is "unchanged" is exactly the one a round is tempted to
quote from memory, and the run is what distinguishes an unchanged figure from an
unexamined one. The same run's status-cell pass is quoted at §9's row for this
round.
...
| **T-3** | **A source-side stall scheduler** — the machinery family G needs | **THE PRIMITIVE EXISTS; THE ORACLE DOES NOT, and the split is `AP-M03` §7's X-1 split at this port.** *(i) Placement*: `test/axi64_probe/axi64_driver.ml` drives one `Axi64.Source` record onto the six `Bits.t ref`s per call, so withholding a word on a chosen cycle is a bench presenting `tvalid` = 0 there — **no new module is needed and none is commissioned**, which is the answer §0.1(iii) obliges me to reach by reading the driver rather than by inferring from §9 that the stimulus is constructible because it is specifiable. *(ii) Computed outcome*: nothing derives, from a stall schedule, the expected strobe cycle, the `/E/` word's cycle and the truncated octet count. **Family G hand-derives all three from §9 and §6.1**, which is the half `AP-M03` gates and this plan simply does not have. **DISCHARGED IN BOTH HALVES 2026-08-12 (`J-dv_lead-0194`, landing `91f005d`), and the state cell is moved to the extent measured and no further.** ***(i) the placement primitive*** was already there when this cell was written and the cell says so; what `WO-0083` §5.3 added above it is the **schedule** — `Bench.Stall` (`frame`, `word`, `hold`, `after`) and `run_scheduled`, whose withholding predicate runs **over the presenter's own cursor and never over a cycle**, so the schedule targets the same *word* whatever the design does and §4.2 fact 1's `R` stays a derivation the unit checks rather than an assumption the bench made. ***(ii) the oracle*** is `WO-0083` §4 — eight facts derived by hand from SPEC-M04 §9 and §6.1, each cited to the specification's own text, fixed in the packet **before** the round rather than delegated to the worker, and **read independently against the spec by architect_docs_lead at `J-architect_docs_lead-0055` (landed `8ceb973`) before the return was reviewed: no fact disputed, no expected value moved.** All three quantities this cell named as underived are now derived and green: the **expected strobe cycle** (`R = S_j + w − 1`, asserted at its pin by six units), the **`/E/` word's cycle** (`A = R + 2`, asserted as two named cycles *and* their difference at `M04-G2`), and the **truncated octet count** (`8w`, with **no FCS and no pad** — 8, 760, 32 and 32 octets at the four members driven). **What this discharge does NOT carry**: `T-7`'s still-missing controllable-release half (untouched, and its cell is left exactly as `J-dv_lead-0192` left it); `T-2`'s conservation monitor, which this round makes the case for **stronger** rather than weaker, because §4.3's tail-frame is a live instance of the `tlast`-keying defect obligation 3 warns about in the abstract; and any multi-stall schedule, which no row has yet needed. **ONE LIMIT OF THIS DISCHARGE, added 2026-08-22 (`J-dv_lead-0200`, the `WO-0083` Revision B item (k)) — the discharge is NOT withdrawn and the state cell does NOT move; what moves is that the limit is now written where a later round meets it.** The schedule-fidelity postcondition `ST-2` — *"for every cycle of the run, `offered.tvalid` equals the bench's own intention record"* — compares two quantities **computed from the same expression in the same branch of the driver**, so it **cannot fire against the driver as written**: it is a **regression tripwire** against a later desynchronising edit, and the defect its own specification names (*"a driver that quietly failed to withhold"*) is precisely the one a self-comparison cannot see, because that failure moves both sides together. **Consequence for the discharge**: the *schedule* is landed and the *oracle* is landed — both halves stand — but **the check that the schedule was executed as written is weaker than its text reads**, and every silence and every strobe assertion downstream of it rests on a stimulus that is asserted by construction rather than by measurement. **The stronger form, for the round that next opens this machinery**: a **count-and-position** check — the number of withheld cycles equals `hold`, and the first of them is the cycle the cursor reached the target — both terms taken from the **schedule** and read from the **drive record**, so a driver defect moves them **independently** (a driver that did not withhold gives 0 against `hold`; one that withheld at the wrong cursor gives the right count at the wrong first cycle). **Executor: dv_lead or tb_writer under a dv WO, in the round that next edits `test/xgmii_tx_64/bench.ml`** — it is a bench edit with a CI-executed result, which is why a packet revision could record it and not land it | family G |
...
| **T-7** | **A direct-drive continuous source with a controllable handover cycle** — the machinery `M04-G10` needs, added 2026-08-11 | ~~**DOES NOT EXIST, and T-3 is not it.**~~ **STRUCK 2026-08-12 — true when written at `9a596e7`, false at `65ba148`. Read this cell's closing paragraph first: the continuous source EXISTS; what does not exist is the CONTROLLABLE half. The struck words are kept because the sentences after them state what the absence was measured against, and a measurement deleted cannot be re-checked.** T-3's primitive drives one `Axi64.Source` record per call, which is enough to *withhold* a word on a chosen cycle; what is missing is the other polarity — presenting the **next** frame's first word while the previous frame is still transmitting, so that its acceptance lands on a **chosen** cycle (`C + 8`, §7's C-16 acceptance) and the frame it opens starts from a word the module already holds. **Measured at this commit, not inferred** (§0.1(iii)): T-3's own capability read establishes the withholding half, and the absence of the other half is read off the committed producer — `test/xgmii_tx_64/bench.ml`'s standing conservation check **fails unless exactly one frame is decoded per run** (`assert_instruments_clean`, the `| fs ->` branch), so no committed unit can drive a second frame at all, let alone hand one over on a chosen cycle. **Neither family D nor REQ-209's sustained run supplies it**: family D drives no handover, and REQ-209's frames are minimum-length, `W = 8`. **Executor: dv_lead**, in the round that first opens a back-to-back bench at M04 — and it is what §0.2 item 4's bar turns on, so it is a **sign-off dependency and not a nicety**. **PARTIALLY DISCHARGED 2026-08-12 (`J-dv_lead-0192`), and the state cell is moved to the extent measured and no further.** ***What now EXISTS***: `test/xgmii_tx_64/bench.ml`'s **`run_stream`** — one elaboration for a run of frames, offering each next frame's word 0 on the cycle after the previous frame's last acceptance, with `wire_frames` and `assert_instruments_clean_n` making the multi-frame decode and the conservation rule readable at `n` frames. Landed by `WO-0082` at `65ba148`, CI-green, and exercised by five units over sixteen runs. **The `| fs -> failwith …` branch this cell cited as the measurement of the absence is still there and still correct** — it is now the `n = 1` special case of `assert_instruments_clean_n`, which is why the sixteen units that predate the round are its regression witness. ***What is STILL MISSING***: the **controllable** half — a release schedule that can place a handover on an arbitrary chosen cycle. Its only consumers are family H's rows (`M04-H4`'s *"whether that word was accepted at C+8 or at C+11"* needs the alternative release), and a capability lands with its first consumer. **`M04-G10` never needed it**: a continuous source presents the next frame's word 0 at exactly `C + 8` by construction, which is a fact about `BUG-0004`'s routes 2 and 3 rather than a convenience — it is why they were reachable in the composed chain's absence and why nobody had driven them. **`WO-0082` §5.4 named this residue BEFORE the round ran**, so the partial discharge is recorded as partial by both documents and not reconstructed after the fact | M04-G10 (and, on the same stimulus, the never-measured routes 2 and 3 of `BUG-0004`) |
| **T-8** | **A `cfg_ifg`-driving harness knob** — the capability `M04-F3` and `M04-F7` need to present any configured gap other than the default, added 2026-08-12 with the `WO-0084-S2` close | **DOES NOT EXIST, measured at this tree** (§0.1(iii), the polarity rule paid at the producer and not inferred from the spec): `test/xgmii_tx_64/bench.ml`'s `create ()` drives `i.cfg_ifg := bits_of_int ~width:8 12` as a **constant** through and after reset, `bench.mli` documents `cfg_ifg = 12` as driven throughout, and every unit's decoder is built `Tx_decoder.create ~ifg:12`, so **no committed producer can present `cfg_ifg ≠ 12`** — the seal's own census (`cfg_ifg = 12` on every cycle of every unit, `J-dv_lead-0195` §2) is a property of this harness, not of any one unit, and it is the whole cause of `class-11`'s survival and `IC-10`'s un-scored survive. **No row is *blocked* structurally** — the stimulus is a single input value and the abort-then-normal shape already exists (`run_stream` / `run_scheduled`, T-3/T-7's landed halves) — but the value cannot be *driven* today, and the decoder's own REQ-204 arm (`~ifg`) must be re-created to track the driven value or its gap-legality threshold is wrong; `bench.ml`'s own run-length note says a round changing `cfg_ifg` must re-derive `g_max`. **Executor: tb_writer**, via the carrier WO (`WO-0085` placeholder, this round's `WO-0084-S2` close); when it lands, `M04-F3` (class-11) and `M04-F7` (IC-10) discharge and the campaign's `class-11` / `IC-10` re-run is enabled (seal §10.6). Recorded where §7 records capability rather than left in a journal. **THE MEASUREMENT ABOVE IS NOW FALSE, AND IS KEPT RATHER THAN OVERWRITTEN so the polarity read and its date stay legible: DISCHARGED 2026-08-22 at `a0cf4dd`** (`WO-0085`, tb_writer `J-tb_writer-0047`, reviewed at `J-dv_lead-0199`), **re-measured at this tree at the producer and not inferred from the packet**: `test/xgmii_tx_64/bench.ml`'s `create` is `create ?(ifg = 12) ()`, drives `i.cfg_ifg := bits_of_int ~width:8 ifg` through reset, stores `ifg` in `Bench.t` and re-drives it from `t.ifg` at `sample_cycle`'s own choke point, and constructs the standing decoder `Tx_decoder.create ~name:"M04 tx" ~ifg ()` **from the same field**, so the driven value and the REQ-204 arm that judges it cannot drift; `run_stream` and `run_scheduled` carry the same `?ifg` (default 12) through to `create` and to the run-length helpers, and `bench.ml`'s own run-length warning is discharged by a `g_max ~ifg = ⌈(ifg + 7)/8⌉` helper that the two `cycles_for_*` functions are re-expressed over — at `ifg = 12`, `g_max = 3` and every term is the arithmetic those functions carried before, which is why the default path is byte-identical. **Two limits of this discharge, measured and stated rather than left to be discovered.** *(a)* `run_one_frame` (hence `run_frames` / `run_lengths`) keeps `create ()` and needs no knob: a one-frame run completes **no** gap (the decoder exempts the first frame, §6.3 item 4), so `ifg` is not observable there — the knob reaches **every runner that can complete a gap** and no further. *(b)* `wire_frames`' own **content-reader** decoder is still constructed `~ifg:12`, and that hardcode is **inert by construction, not by luck**: `Tx_decoder`'s `ifg` feeds the REQ-204 `violate` branch **only**, and `wire_frames` returns `Tx_decoder.frames` and never reads that instance's `report` or `is_clean`, so no value it yields depends on `ifg`. **It becomes live the moment any consumer reads that instance's report**, and a round that does so must thread the knob there before it does | M04-F3, M04-F7 |
...
7. **Machinery T-7 and the two never-measured routes of `BUG-0004`** (added
   ...
   **RESOLVED IN PART, 2026-08-12 (`J-dv_lead-0192`).** ~~*"and no such producer
   exists — the committed bench cannot decode two frames in one run at all"*~~ —
   **struck: it was true when written and is false at `65ba148`.** `WO-0082` built
   the continuous source and drove `M04-G10` at four members; §0.2 item 4's
   permitted form **(i)** is satisfied and the item's satisfaction note is
   recorded there. **What survives of this item**: `T-7`'s **controllable** half
   is still unbuilt (§7's state cell carries the residue and names family H as its
   only consumer), so this item stays **OPEN as a machinery debt** and closes
   **not** as a gate condition — that half of it is discharged. **Mine, executor
   unchanged.**
...
| 2026-08-11 | **The second M04 bench round is ABSORBED: TWELVE ROWS DISCHARGED at `aabae58` — ten ASSERT by a green assertion and two NO-ASSERT by a prohibition that held. Families D and E are COMPLETE, the first two families to close at this module.** `M04-D1`, `M04-D2`, `M04-D4` (U11, `test/xgmii_tx_64/test_m04_d.ml`, the directed set `P ∈ {1, 20, 59, 60, 61, 64, 67, 1514}` — D1 the four wire octets at `F−4 … F−1` against `Frame.fcs (Frame.pad_to_60 (content_octets ~p))` octet for octet in wire order, D2 `Frame.residue_ok` over the whole decoded frame as **corroboration and expressly not a second REQ-202 anchor**, D4 each FCS octet's own `(cycle, lane)` read off the raw sample plus the `P = 60` / `P = 64` contrast); `M04-D5` (**NO-ASSERT**, U11 title + round-wide — no unit, comment, title or verdict sentence asserts the CRC **enable** or its `octet_count`, and ADR-0007's mechanism is **not** claimed as behaviourally checked at this module); `M04-D3` (U12, two `P = 60` frames differing in octet 0 alone, anti-vacuity asserted first and by itself); `M04-D6` (U13, the all-zero 60-octet frame, the oracle's value computed at run time and **printed**, asserted non-zero against the seed-instead-of-finished-value defect); `M04-E1`, `M04-E2`, `M04-E3` (U14, `test_m04_e.ml`, the eight-lane sweep `P ∈ {60 … 67}` — **one cycle, eight lanes**, all eight terminating at `C + 10` with only `t = F mod 8` moving; E2's fill asserted as a **value** (control bit set **and** `xgmii_txd` = `0x07`) over the terminate word's lanes `t+1 … 7` and every lane of cycles `terminate_cycle + 1 … run_length − 1`; E3's boundary pair `t = 7` zero fill lanes / `t = 0` seven, the zero **counted** rather than left to an empty loop); `M04-E5` (**NO-ASSERT**, U14 title + round-wide — every scan reads **that run's own** `terminate_cycle`/`terminate_lane` and no helper is written as a universal over terminate characters, so no assertion here fails a conformant M04 on an underflowed frame); `M04-E4` (U15, `P = 1514`, `/T/` at lane 6 of `C + 2 + 189` — the row whose content is `⌊F/8⌋ = 189` and not the lane); `M04-G9` (U16, `test_m04_g.ml`, `P ∈ {1, 8}`, `W = 1` at both — the frame transmits intact **and** the `error_underflow` strobe's exact event set is **empty**, called first and named as the row's own assertion rather than as a background check). Carrier `WO-0081` (`RV-0081-VERDICT` **ACCEPT**, 20 of 20 bars, `J-dv_lead-0179`; no bounce condition fired; **`BM17` ARMED and NOT tripped** — zero instruments outside §17.1's allow-list, disclosed or otherwise). **`M04-G9`'s discharge is the adjudicating instrument the row above deliberately withheld**: `J-dv_lead-0177` recorded that the landed bench's standing instruments already met this row's strobe half at `af06c62` and refused to count it, because a row written after a run is not discharged by that run merely because the run would have passed it. This round gives the row its own stimulus, its own unit, its own content assertions and a verdict, and **that** is what discharges it. Evidence: CI `build` run **31494947078**, job `build` **93790202820** at `aabae58`, steps *Build* `success` and *Run tests (expect tests, waveform snapshots)* `failure`, read by name and status; the failure is **§15 class P**, the pre-committed promotion of U13's `[%expect]` block and **nothing else** — `dune promote`'s own `git diff --name-only` in that step names **exactly one** file, `test/xgmii_tx_64/test_m04_d.ml`, whose sole hunk replaces that unit's own empty `[%expect]` block with printed oracle data carrying **no exception text**, so all six units of this round ran and **none of their OCaml assertions fired**, and no other expect test in the repository moved. The same class-P red, byte-identical in its promotion payload, is re-read in full at run **31495302673** job **93791378385** (`cddad51`, rtl_lead's M07 landing, which touches `libs/**` only — `git diff aabae58 cddad51 -- test/` **empty**). **The promotion is placed in this round by dv_lead**, reconstructed from the CI promotion block by two independent channels that converge byte for byte — the base64 payload decoded, and the tree file with that one line substituted — both at sha256 `e1f8e9f0b4abbfa2af7a9b8743cc2e1f9427632e16bb41569397a08496f72979`, the figure CI itself printed, verified under four instruments (`sha256sum`, `openssl dgst`, Python `hashlib`, `shasum -a 256`); and **the printed value was checked against an oracle outside this repository before it was accepted as an expectation** — `CRC32` of sixty zero octets is `0x04128908`, i.e. `0x08 0x89 0x12 0x04` least significant octet first, agreeing under Python `zlib.crc32` and an independently written bit-serial routine over the reflected IEEE 802.3 polynomial `0xEDB88320`, both anchored on REQ-303's published check value `0xCBF43926`. **The completing event is named and not assumed**: the *Run tests* and *Verify nothing was left unpromoted or non-deterministic* steps are `failure`/`skipped` until the commit carrying this promotion runs green. **If that run is not green at both steps, this discharge is VOID and these twelve rows reopen** — the condition is recorded here rather than left to be inferred, because a discharge whose completing run was never read is the shape `FINDING WO-0080-2` convicted. **RESOLVED IN PLACE, 2026-08-11 (`J-dv_lead-0180`): the void condition did NOT fire and these twelve rows stay discharged.** The completing run is CI `build` run **31499963043**, job `build` **93807078426**, `head_sha` **`8d70da1`** — the commit carrying the promotion — `run_attempt` **1**, job conclusion `success`, with step **6** *Run tests (expect tests, waveform snapshots)* `success` and step **8** *Verify nothing was left unpromoted or non-deterministic* `success`, read by name, number and status from the job record at the source and not from a badge or a summary; every other step of that job is `success` too. `git show 8d70da1 --stat` names `test/xgmii_tx_64/test_m04_d.ml` and the promoted `[%expect]` line is present at that SHA, and `git diff 8d70da1 HEAD -- test/` is **empty**, so the green run reads these benches exactly as they stand at this commit. **No row, status cell, count or coverage-map line moves on this resolution: the counts above stand as measured — 82 rows, 58 ASSERT, 12 NO-ASSERT, 6 NO-STIMULUS, 5 STRUCTURAL, 1 GAP, 0 RULING, outstanding 57 of 82.** The resolution is written onto the claim rather than left in the entry that read it, because a conditional discharge whose completing event lives only in a journal makes every later reader of this plan re-derive it; `WO-0081`'s `RV-0081-VERDICT` §6 carries the identical annotation in the same act. **NO ROW ADDED, NO ROW CONVERTED, NO STATUS CELL MOVED, NO COVERAGE-MAP LINE CHANGED, NO CELL OF ANY ROW TOUCHED: 82 rows (82 distinct ids), 58 ASSERT, 12 NO-ASSERT, 6 NO-STIMULUS, 5 STRUCTURAL, 1 GAP, 0 RULING** — re-measured at this tree by a status-cell pass over every row table, before and after this edit, and **not** carried forward. The twelve discharged here measure **ten ASSERT + two NO-ASSERT** by that same pass, which is `WO-0081` §2's own figure and which corrects the carrier's worker journal (`J-tb_writer-0044`, Trigger) where the same split is written *"eleven ASSERT + one NO-ASSERT"* — a count in a narrative sentence, contradicted by that round's own `dune` header row, which states ten and two correctly; nothing in the work depends on it. **The discharge count remains a HAND COUNT with its provenance**: `tools/dv_checks.sh` at this commit still contains **zero** occurrences of `M04`, `xgmii_tx_64` or `AP-xgmii_tx`, so **no committed instrument counts an M04 row** and `DVC-1a` is still owed and still load-bearing for any coverage fraction. **Outstanding moves from 69 of 82 to 57 of 82.** **Three defects were reported by the carrier's worker rather than adopted, and are adjudicated at `RV-0081-VERDICT` §4**: two UPHELD against `WO-0081` (its §6.1 prose and §6.2 assertion-6 cell call wire octet 59 at `P = 60` a *"pad octet 59 (`0x00`)"* where `P = 60` has pad count **zero** and that octet is the last **content** octet, value `0x3C`; and *"eleven exported values"* of `bench.mli` at **four** packet sites where a status-cell pass measures **twelve**, the enumeration having counted `decoder`/`strobes` as one) and one REFUTED (the §6.1 master table's `P = 67` FCS cell reads *"lanes 3–6 of `C + 10`"*, which is correct and is what the worker's own derivation produced — it misattributed the neighbouring `P = 63` row's cell). **None of the three touches a row of this plan and no cell here moves on any of them.** **`BAR T1` stays SHUT; `§0.2` item 4's REQ-206 bar stays in force; no `SO-xgmii_tx_64.md` is opened or offered; `M04-G4` and `M04-G10` are NOT discharged and are not described as such; REQ-206 is NOT described as covered.** | dv_lead, `J-dv_lead-0179` |
| 2026-08-12 | **The third M04 bench round is ABSORBED: SIX ROWS DISCHARGED at `65ba148` — five ASSERT by a green assertion and one NO-ASSERT by a prohibition that held — and this is the first round of this module to measure a GAP, a HANDOVER, or any run containing two frames.** `M04-F1` (U17, `test/xgmii_tx_64/test_m04_f.ml`, two `P = 60` frames from one elaboration — the **first inter-frame gap this programme has measured at a transmit port**: `Tx_decoder.gaps` has exactly one entry and it is **16** octets from the terminate character **inclusive**, `start_spacings` exactly one entry and it is **11** cycles = **88** octets, REQ-204's own verification figure, **each list's length asserted first and by itself** so a two-gap run cannot pass a one-gap check; and the fill residue `WO-0081` §19.2 item 4 named is paid — the terminate word's lanes 1…7 carry `/I/` **as a value** (`0x07`, not merely a control bit) and cycle `C + 11` is all-idle, with the scan **stopping before `S₂ = C + 12`** because the next preamble word is not a gap word); `M04-F2` (U18, the eight-member terminate-lane sweep `P₁ ∈ {60 … 67}`, gaps **16, 15, 14, 13, 12, 19, 18, 17**, **driven whole and not sampled** — §0.3's convention and the convention §0.3 rejects agree at seven residues and differ **only at `t = 4`**, where this specification gives **12** octets and the rejected reading **20**, so a sampled sweep has a seven-in-eight chance of missing the one member that discriminates; the landed unit asserts **12** against a live design and passes); `M04-F5` (**NO-ASSERT**, round-wide — no unit asserts an average gap, none asserts *"12 octets at `t = 0`"* (the value there is **16**; 12 is `cfg_ifg`, the minimum), and §0.3's **receive**-side spacing is imported nowhere; stated in `test_m04_f.ml`'s own docstring and in both F units' comments, which is where a reader of the tests meets it rather than only in a packet); `M04-A3` (U19, one elaboration, **100 consecutive `P = 60` frames**, 1 227 cycles — exactly **100** start cycles asserted as its own statement because REQ-201's §10 hook *is* the count, every one with `xgmii_txc` = `0x01`, `start_lane = Some 0` and the identical preamble word compared by `List.equal Int.equal`, **with the frame index in every failure message**; and `start_spacings` is read nowhere — a hundred frames at an 11-cycle cadence is REQ-209's sustained claim and belongs to a row this round did not carry); `M04-B3` (U20, **both orders**, `1514 → 20` and `20 → 1514`, each frame's decoded octet count its own `F` — 1518 and 64 — each terminate character at its own `Int.rem F 8` computed in the unit, each frame's octets equal to the REQ-305 oracle's `Frame.with_fcs (Frame.pad_to_60 …)` for **its own** `p`, so a CRC register or length counter not re-seeded between frames dies in whichever order it manifests; the row's Kills cell says in terms that one order tests one of the two defects); `M04-G10` (U21, four runs — shape **(a)** `W = 1` at `B ∈ {1, 8}` (`BUG-0004` route 2) and shape **(b)** `W = 2` fully pre-loaded at `B ∈ {9, 16}` (route 3) — with **the route's own precondition asserted FIRST and as a whole list**: nine acceptances `[C … C+8]` at (a), ten `[C … C+8; C+11]` at (b) **with the hole at `C+9` and `C+10`**, the void language in its failure message; then the **named silence** at `C + 12` (a) and `C + 13` (b), the cycles the unfixed design strobes at by `BUG-0004` §9.3's derivation; then `S_B = C + 12`, **unmoved by the early acceptance** (§7's C-16 consequence 3); then both frames intact). Carrier `WO-0082` (`RV-0082-VERDICT` **ACCEPT**, **23 of 23 bars**, `J-dv_lead-0191`; **no bounce condition fired**; **`BM17` ARMED and NOT tripped**, the arming measured from the worker's own factual report of its dispatch rather than inferred; **two disclosures adjudicated on their merits** — the mid-round docstring self-repair that a worker's own `M04-<letter><digits>` sweep caught before CI and before review, **credited in full, no defect**, and the `ocamlc` shell plumbing the worker disclosed against its own interest, **ruled INSIDE §17.1 item 3** on purpose, necessity and consequence, with the boundary written into `WO-0082` §17.1 at its Revision B so no later seat has to derive it). **Evidence, re-read at the source at this absorption and NOT carried from the verdict**: CI `build` run **31539904673**, job `build` **93939708019**, `head_sha` **`65ba148`**, `run_attempt` **1**, job conclusion `success`, read from the job record by step **name, number and status** — step **5** *Build* `success`, step **6** *Run tests (expect tests, waveform snapshots)* `success`, step **8** *Verify nothing was left unpromoted or non-deterministic* `success`; every other step of that job `success` too. **`journal-check` run 31539904725** at the same `head_sha`, `run_attempt` **1**, conclusion **`success`** (R1–R8 re-verified over the pushed range). **`git diff 65ba148 HEAD -- test/` is EMPTY at `1b684c7`**, so the green run reads these benches exactly as they stand at this commit. **§15's class P was pre-declared unreachable for this round and was not reached**: no printed value was commissioned, every `[%expect]` block the round wrote is `{||}`, and a green *Run tests* beside a green *unpromoted* check is the first round of this chain to land the prediction its own packet demanded be stated before the run. **COUNTS, re-measured at this tree by a status-cell pass over every row table, before and after every edit, never carried forward: 82 rows (82 distinct ids), 58 ASSERT, 12 NO-ASSERT, 6 NO-STIMULUS, 5 STRUCTURAL, 1 GAP, 0 RULING.** §6.1's homing census re-run with its own quoted command: **82 82 []**, unchanged, and re-run rather than quoted. **NO ROW ADDED, NO ROW CONVERTED, NO STATUS CELL MOVED, NO COVERAGE-MAP LINE CHANGED. Outstanding moves from 57 of 82 to 51 of 82.** **Four cells DID move and are enumerated, because *no cell touched* would be false this time**: `M04-G10`'s **Stimulus** cell (*"Needs machinery T-7 and is not mountable at this commit"* — **struck in place**, true when written and false at `65ba148`); §7 item **`T-7`**'s state cell (**PARTIALLY DISCHARGED**, the continuous source and handover landed, the **controllable** half still unbuilt with family H named as its only consumer); §0.2 item **4** (permitted form **(i)** recorded **satisfied**, which `WO-0082` §9.8 permits only here, together with the four things it does not do); and §8 item **7** (its *"no such producer exists"* clause struck, the item surviving as a machinery debt and no longer as a gate condition). **One wording of my own verdict is corrected in the paying**: `RV-0082-VERDICT` §6 owed *"the six rows' Status cells"*, and **this plan has no such act** — a `Status` cell holds one of §1's six vocabulary values, **discharge is not a status value**, and both prior absorptions record in terms that no status cell moved. The act actually owed is this row, and it is what discharges the six. **The discharge count remains a HAND COUNT with its provenance**: `tools/dv_checks.sh` at this tree still contains **zero** occurrences of `M04`, `xgmii_tx_64` or `AP-xgmii_tx`, so **no committed instrument counts an M04 row**, and `DVC-1a` (§8 item 5) is still owed and still load-bearing for any coverage fraction a sign-off would quote. **A machinery finding is recorded here because the plan is what a stage-2 packet reads**: the bench's stream postcondition **`SP-2`** (accepted words = words offered) and its **`assert_instruments_clean_n`** (strobe events **0**, **no** decoded frame underflowed) are **both false by construction against a conformant M04 under family G's own stall stimulus** — §9's abort ends the frame, so the withheld word is never accepted, one strobe cycle is the pinned conformant outcome and one decoded frame is legitimately `underflowed`. The replacement class is designed at `WO-0082` §20 (Revision B, `J-dv_lead-0192`) and stage 2 inherits it; **the second of the two instruments was found at that design round and not at the verdict**, which is why §20 also carries the procedural rule that a new stimulus class obliges a re-reading of **every** run-level assertion the new runner inherits. **`BAR T1` stays SHUT** — no transmit reference vendored, no transmit harness, REQ-901 declaring no divergence class — so every figure this round measured is **bench-only**; **no `SO-xgmii_tx_64.md` is opened or offered**; **`M04-G4` is NOT discharged**; **REQ-206 is NOT described as covered** and `M04-G1` … `G8` remain outstanding; **no family-H row is discharged** though three were driven past; and **family F does NOT complete** — `F3` needs the configuration axis, `F4` family I's sustained run, `F6` family G's stall schedule. **The mutation campaign for these six rows is sequenced after this ACCEPT and before any `SO-` PASS (PROTOCOL §10) and is the orchestrator's to schedule.** | dv_lead, `J-dv_lead-0192` |
| 2026-08-12 | **The fourth M04 bench round is ABSORBED: EIGHT ROWS DISCHARGED at `91f005d` — all eight ASSERT by a green assertion — and this is the first round of this module, in either design, to drive an UNDERFLOW at all.** Every M04 run before this one presented every word the transmitter asked for; twenty-eight runs and thirty-one discharged rows had produced no strobe and no underflowed frame, which is what `T-3` was the measurement of. **`M04-G5`** (U22, `test/xgmii_tx_64/test_m04_g.ml`, `run_scheduled [ content_octets ~p:60 ]` withholding word 1 for one cycle and abandoning — the **earliest cycle REQ-206's condition can hold**: the strobe at its derived pin `R = C+1`, the `/E/` `/T/` word at `A = R+2 = C+3` with **all eight lanes** asserted because `xgmii_txc` = 0xFF is part of §9's shape, the aborted frame **8 octets** on the wire and **not 60 and not 64**, `terminate_lane` 1, `terminate_cycle` `C+3`, and `gaps` **empty** because an abandoned abort completes no gap); **`M04-G1`** (U23, `P` = 1514, word 95 withheld and resumed — **760** octets on the wire, the length asserted before the content, **no FCS and no pad**, and the gap served from the `/E/` `/T/` word); **`M04-G2`** (U23 — the **separation**, read independently from the sample record as two named cycles and then asserted as their difference: strobe `C+95`, `/E/` `C+97`, difference **2**. The design this row exists to kill pulses at the wire consequence, which is one pulse, one `/E/` word, and **inside** §0.6's window — a bench asserting only that both happened passes it, and this is the assertion that does not); **`M04-G8`** (U23 — two frames on the wire with **exactly one** underflowed at position 0, and the run's **one** `tlast` acceptance at a cycle later than the aborted frame's own terminate cycle: under `Resume` the withheld frame's `tlast` word closes the **tail**, so a conservation rule keyed on `tlast` counts one frame where two reached the wire, which is obligation 3's abstract warning made concrete by a stimulus); **`M04-G3`** (U24, word 185 withheld for **four** consecutive cycles and resumed — **exactly one** pulse, at the first of them, carried by the monitor's exact-event-set check; silence named at each of the three later withheld cycles so a failure is diagnosable by cycle; **exactly one** error character in any lane of the whole run; and the gap **23** and not 15, because `hold = 4` puts the resume past the gap's last cycle and §4.2 fact 7's branch (b) applies — the two abort gaps this round measures from the same abort word differ, and both are asserted); **`M04-G6`** (U25, an aborted frame followed by a `P` = 20 frame — the second frame's full preamble word from the raw wire, its 64 decoded octets equal to `Frame.with_fcs (Frame.pad_to_60 (content_octets ~p:20))` with the pad **inside** the FCS computation, and terminate at lane 0. A design whose CRC register is not re-seeded after an abort has the right length, the right pad count and the right terminate lane, and **only this comparison speaks**); **`M04-F6`** (U26, `test/xgmii_tx_64/test_m04_f.ml` — `Tx_decoder.gaps` **exactly one** entry and it is **exactly 15**, never `≥ cfg_ifg`, ~~the wrong design being **one octet** from conformant~~ **[STRUCK IN PLACE 2026-08-22, `J-dv_lead-0200`: this row is a dated record of what the `WO-0083` round claimed and its text is NOT rewritten — the strike marks the claim false and leaves the record readable. `WO-0084-S1` (`J-dv_lead-0196`, absorbed `J-dv_lead-0197`) struck the same claim from `M04-F6`'s Kills cell: the decoder counts from the wire's `/T/`, so every abort gap it can record is `≡ 7 (mod 8)` and 16 is not in the set; at this row's `cfg_ifg = 12` the `/E/`- and `/T/`-readings are byte-identical and the real kill is the **word count**, 7 or 23. Found by the site rule `FINDING WO-0085-R1` banked (`J-dv_lead-0199`) — enumerate sites by the claim, not by the string — which reaches a change-log row that no sweep for the figure `16` could]**; and the terminate character read at **lane 1** twice, from the raw wire and through the decoder, deliberately, because this is the **first gap in the programme whose terminate character comes from an aborted frame** and a disagreement between the two readings would have been an instrument defect rather than a design one); **`M04-A4`** (U27, `test/xgmii_tx_64/test_m04_a.ml` — three predecessors in one run, `clear`, a normally terminated frame and an **underflowed** frame, the three preamble words equal **to each other** and **to the REQ-201 literal**, both comparisons because three identical wrong preambles pass the first and fail the second; `start_cycles` exactly `[C+1; C+12; C+19]`; and `gaps` `[16; 15]`, the same design serving 16 after a clean terminate at lane 0 and 15 after an abort's at lane 1 **in one run**, which the family-F row alone cannot show). **The counts, by the method the plan uses and not by recall**: the three prior absorption rows' own declared counts (13 + 12 + 6 = **31**) plus this round's **8** = **39 discharged**, against a row total of **82** re-measured at this commit by a status-cell pass (58 ASSERT, 12 NO-ASSERT, 6 NO-STIMULUS, 5 STRUCTURAL, 1 GAP) — so **51 → 43 outstanding**. **`FAMILY A COMPLETES HERE`, the third family to close at this module after D and E** — verified by a census over the family's five rows rather than asserted: `A1`, `A2`, `A5` at `af06c62`, `A3` at `65ba148`, `A4` here. **What this round does NOT do, restated because it is the paragraph most likely to be over-read later**: **family G does NOT complete** — `M04-G4`, the faithful implementation read to its first full stop, is held back **for one round only and needs nothing built**, and `M04-G7` remains NO-ASSERT and standing; **family F does NOT complete** — `F3` needs the configuration axis and `F4` family I's sustained run; **REQ-206 is NOT covered** and no artifact of this round describes it as covered; **no family-H, -I, -K or -L row is discharged** though this round drove past three families' cycles; **`BAR T1` stays SHUT** — no transmit reference vendored, no transmit harness, REQ-901 declaring no divergence class at this boundary — so every figure this round measured is **bench-only**, and a fully opened lane would not have seen `error_underflow` in any case, which leaves this round's headline claim bench-only either way; **no `SO-xgmii_tx_64.md` is opened or offered**. **The mutation campaign over these eight rows is sequenced after this ACCEPT and before any `SO-` PASS (PROTOCOL §10) and is the orchestrator's to schedule.** | dv_lead, `J-dv_lead-0194` |
| 2026-08-12 | **The two WO-0084 campaign follow-ups are absorbed — the unreachable figure (`WO-0084-S1`) and the config-coverage debt (`WO-0084-S2`) — and this is the first row of this plan whose subject is a mutation campaign's own findings rather than a bench round.** **`WO-0084-S1` (MINOR, mine, PRE-COMMITTED at the seal §11.7 and filed at act 4, `J-dv_lead-0196`):** `M04-F6`'s Kills cell named a `/E/`-vs-`/T/` design's output as *"(`t = 0`, gap 16) … one octet from conformant"*; **that `16` is arithmetically unreachable** — `Tx_decoder.gaps` measures from the wire terminate character, which §9 fixes as the `/T/` in lane 1, so every abort gap it can record is `8k − 1`, i.e. `≡ 7 (mod 8)`, and 16 ∉ {7, 15, 23, …}. **The figure is struck in place with its date and grounds** (`FINDING WO-0084-S1` / `J-dv_lead-0196` / seal §6), never overwritten; the corrected cell states the row's real kill (the **word count** — 7 or 23 — via the exact `= 15` assertion) and **re-homes** the `/E/`-vs-`/T/` (IC-10) coverage to `M04-F7` rather than dropping it. **The identical `16` in `test/xgmii_tx_64/test_m04_f.ml`'s `M04-F6` comment/message is the finding's second site (seal §6) and is folded into the carrier WO — this round's write scope excludes the bench.** **`WO-0084-S2` (coverage debt, mine):** the M04 suite drove `cfg_ifg = 12` on every cycle of every unit (the seal's §2 census), so `class-11` (gap-ignores-`ifg`) survived the whole campaign and the sealed `IC-10` (its abort sibling) could not be scored. **`M04-F3` already carried the normal-gap class-11 killer** (ASSERT, `cfg_ifg ∈ {12, 13, 16, 20, 255}`) and is **NOT duplicated**; **ONE ROW IS ADDED — `M04-F7`, family F 6 → 7, ASSERT** — the abort-gap-tracks-`cfg_ifg` row that kills IC-10 and re-homes the coverage `S1` struck from `M04-F6`, driving an abort at `cfg_ifg ∈ {12, 16, 24}` where the `/E/`-vs-`/T/` reading separates by a whole word at 16 and 24. **§7 gains `T-8`** — the `cfg_ifg`-driving harness knob, **measured absent at `test/xgmii_tx_64/bench.ml`** (`create ()` hard-drives `cfg_ifg := 12`; decoder `~ifg:12`), §0.1(iii)'s polarity read done at the producer and the file named — with **tb_writer** the executor via the carrier. **CARRIER NAMED:** a new tb_writer WO, `agents/handoffs/WO-0085_tb-m04-cfg-ifg-gap-axis.md` (placeholder id, orchestrator allocates at commit), commissioning the `cfg_ifg` knob, the `M04-F3` and `M04-F7` units, and the `M04-F6` bench-comment correction; whether the bench is written now or a later round is the tb_writer round's affair — the DEBT closes here, the plan carrying the rows and the carrier named. **COUNTS, re-measured at this tree by a status-cell pass over every row table, before and after every edit, never carried forward: 83 rows (83 distinct ids), 59 ASSERT, 12 NO-ASSERT, 6 NO-STIMULUS, 5 STRUCTURAL, 1 GAP, 0 RULING** — the sole delta is the one new ASSERT row. §6.1's homing census re-run with its own quoted command: **83 83 []** (re-measured 2026-08-12; `M04-F7` reached through §6's REQ-204 range `M04-F1 … M04-F7`). **Cells that MOVED, enumerated because *no cell touched* would be false this round**: `M04-F6`'s Kills cell (the `S1` correction), `M04-F7` added, §5 item 12 (its rejections), §6's REQ-204 / REQ-206 / REQ-802 lines (F7 homed), §6.1's census output and its re-measurement note, §7's `T-8`. **NO pre-existing row's Status cell moved; no row was converted; `M04-F3` was not duplicated.** **Outstanding moves from 43 of 82 to 44 of 83** — `M04-F7` is added undischarged, and `M04-F3` was already among the 43 outstanding. **`BAR T1` stays SHUT; no `SO-xgmii_tx_64.md` is opened or offered; `M04-G4` is not discharged; REQ-206 is NOT described as covered.** **The stage-2 `WO-0083` revision is untouched this round** — its eleven owed items are their own queued round (dispatch item 3); this round is the two campaign findings and nothing else. | dv_lead, `J-dv_lead-0197` |
| 2026-08-22 | **The `cfg_ifg` axis lands and `WO-0084-S2`'s coverage debt is DISCHARGED IN THE SUITE, not only in the plan: `M04-F3` and `M04-F7` are asserted and green, and this is the first round in either M04 design to drive `cfg_ifg ≠ 12` at all.** Reviewed and **ACCEPTED** at `a0cf4dd` (`WO-0085`, tb_writer `J-tb_writer-0047`; verdict row in the packet's Return log, `J-dv_lead-0199`). **`M04-F3`** (U28, `test/xgmii_tx_64/test_m04_f.ml`, `run_stream ~ifg` over two `P = 60` frames per member, `cfg_ifg ∈ {12, 13, 16, 20, 255}`) — `Tx_decoder.gaps` length **asserted first** then the gap **exactly** 16, 16, 16, 24, 256, never `≥ cfg_ifg`, with frame 2's `start_cycle` at `C + 10 + g` as a second and independent reading of the same fact; the standing decoder is now constructed `~ifg` **from the driven value**, so a design under-serving the configured gap reddens at the instruments as well as at the row's own assertion. **`M04-F7`** (U29, `run_scheduled ~ifg` at `M04-F6`'s own stall shape, `cfg_ifg ∈ {12, 16, 24}`) — one abort-gap entry, exactly 15, 23, 31, the `/T/` at lane 1 of the abort word read from the raw wire (invariant in `cfg_ifg`), and the next start character in lane 0 at `A + g`. **Every expected value re-derived from §4's identity per member; none of `M04-F1`/`F2`/`F6`'s `cfg_ifg = 12` figures is carried forward — and the `cfg_ifg = 12` member of each new unit reproduces the landed figure of its family predecessor exactly (`F3`@12 = `F2`'s `p1 = 60` member: gap 16, `C+12`; `F7`@12 = `F6`: gap 15, `C+8`), which is the collapse a generalisation owes its own special case.** **The default path is byte-identical, and the claim is made by the measurement and not by the worker's word**: no `[%expect]` block anywhere in the diff is removed or edited (only new ones added), the other seven `test_m04_*.ml` files are untouched, and CI ran the whole suite — build run **32553311119** SUCCESS at this SHA, whose *Run tests* step is `dune runtest` and whose *Verify nothing was left unpromoted* step is `git add -A && git diff --cached --exit-code`, the DoD's second half in a stronger form than a bare `git diff`; journal-check run **32553311111** SUCCESS. **Local `dune runtest` was NOT run and could not be** (opam blocked by the container network policy, ADR-0005 — this directory's standing posture, stated in its own `dune` header): **CI green is the first and only execution evidence for these two units anywhere**, and the round is accepted on that basis with the derivation checked independently at review, member by member. **`FINDING WO-0085-R1` (MINOR, mine), filed at review and corrected in the same tree:** the `WO-0084-S1` second-site correction reached the three sites that **quoted** the unreachable `16` and missed the `U26` banner that **paraphrased** the same claim (*"the one octet that separates conformant from not"*) — a correction scoped by the expression of a claim misses every restatement of it. Corrected at `test/xgmii_tx_64/test_m04_f.ml`; the `M04-F6` row records it. **COUNTS, re-measured at this tree by a status-cell pass over every row table, never carried forward: 83 rows (83 distinct ids), 59 ASSERT, 12 NO-ASSERT, 6 NO-STIMULUS, 5 STRUCTURAL, 1 GAP, 0 RULING** — **no row is added, converted or struck this round**, so every figure is unchanged and is re-run rather than quoted. §6.1's homing census re-run with its own quoted command: **83 83 []**. **Discharged moves 39 → 41 by the same method the four prior absorption rows used (13 + 12 + 6 + 8 = 39, plus this round's 2), so outstanding moves 44 of 83 → 42 of 83.** **Cells that MOVED:** `M04-F6`'s Kills cell (second site closed, `R1` recorded), §7's `T-8` (its **DOES-NOT-EXIST measurement is now false** — kept and dated rather than overwritten, with the discharge re-measured at the producer and two limits stated: `run_one_frame` needs no knob because a one-frame run completes no gap, and `wire_frames`' content-reader `~ifg:12` is **inert by construction** because that instance's report is never read), §6.1's census note, and this row. **What does NOT move, stated because silence here would be read as movement:** **no Status cell moves** — this plan records a discharge in **this table**, never in the Status cell, whose vocabulary is the six values §1 fixes and which carries no annotation anywhere (the status-cell pass above returns exactly those values; minting a discharge marker in the notation §6.1's census selects is what `FINDING AP-6-2` forbids); **no `SO-xgmii_tx_64.md` is opened or offered**; **`BAR T1` stays SHUT**, so every figure this round measures is bench-only; **REQ-206 is NOT covered and no artifact of this round describes it as covered** — `M04-F7` attacks REQ-206's §9 stream-effect cell at one point and the family-G rows that would carry the requirement are unchanged; **`M04-G4` is not discharged**; the `WO-0083` stage-2 revision items are untouched; and **`WO-0084`'s tally is not edited** — a campaign's score is a frozen measurement (PROTOCOL §7) and what follows sits beside it. **THE `class-11` RE-RUN IS NOW ENABLED AND IS OWED BEFORE `P1-module-ready`** (seal §10.6 item 6, `WO-0084-S2`): PROTOCOL §7 (b.2) gives a survivor exactly one disposition form — the **unmodified** committed diff replayed against the bench as it stands at the gate SHA, at a run id, with the ~~**killing unit named**~~ **[STRUCK AND RE-QUOTED IN PLACE 2026-08-22, `J-dv_lead-0203`. The strike marks the quotation STALE, not the claim false — the struck words were `(b.2)`'s own text on the day this row was written and are not its text at `469c2f3`, which applied `ADR-0024` A3 (`F-0022-2`, countersigned by this seat at `J-dv_lead-0202`); the row's text is NOT rewritten, so what this round quoted stays readable and the applied clause sits beside it. **AS APPLIED, read at the source** (`git show 469c2f3:agents/PROTOCOL.md`, survivor limb): *"the **unmodified** committed diff, replayed against the bench as it stands at the gate SHA, at a **run id**, with the **killing unit or units named** — anything weaker lets a survivor be argued dead"*; and the plural reaches THIS limb, not only the frozen-kill one, by the same clause's *"**In both limbs**, the referent is the campaign record's own naming, at whatever cardinality it named"*. **The consequence for the very disposition this sentence instructs, stated so no later reader re-derives it**: the `class-11` re-run named **two** units — `M04-F3`/`U28` at `cfg_ifg = 20` and `M04-F7`/`U29` at `cfg_ifg = 16`, run `32554521276` — so the `P1-module-ready` disposition carries **both**, and naming `U28` alone is the selection A3 forbids (`J-dv_lead-0202` Construction C). Nothing else in this row moves: no status cell, no count, no census, no discharge figure]** — and that is a disposition check, **not a new campaign**. Procedure, seats and the not-closed criterion are ruled at `J-dv_lead-0199` and recorded in the `WO-0085` Return log; the predicted disposition is **KILL**, first at `M04-F3` (U28) member `cfg_ifg = 20`. **`IC-10` cannot be re-run: it was never rendered** (`sealed`, not `seeded`), so rendering it now is a **first** measurement and needs an **auditor-authored** manifest in a **new** directory — never an edit to `docs/reports/audit/WO-0084-mutations/`, and never one I author, which would be a bench graded by a defect its own author chose | dv_lead, `J-dv_lead-0199` |
| 2026-08-22 | **The `WO-0083` stage-2 packet REVISION is absorbed — the round that stages no test and discharges no row, and the first row of this table whose subject is a CLOSED packet's own text.** `WO-0083` **Revision B** (`J-dv_lead-0200`, at head `6f165bd`) applies the **eleven** items `RV-0083-VERDICT` §5 routed to *"this packet's NEXT revision"*: **six** from architect_docs_lead's independent §4 read (`J-architect_docs_lead-0055`, `8ceb973` — fact 1's citation upgraded to §6.1's storage sentence plus §6.2's `Preamble` row plus C-16 consequence 1's **uniqueness** claim, which is the ground `M04-G5`'s preamble-cycle row actually needs; fact 5 restated as the **law** `g = ⌈(cfg_ifg + 1)/8⌉` with 15 at `cfg_ifg = 12` as its **value**, plus §9's *"the gap is then served from that terminate character"* as the citation that fixes **which** character starts the count; fact 7's stability ground moved from §7's handshake bullet — which is **acceptance-cycle-only** — to the round's **own presenter**, no document in this programme obliging a source at this port to hold until accepted; §4.4's `word ≥ 2` re-grounded on C-16 consequence 4's **second** branch, where word 1's required-ness is **design-dependent**, because the frame after an abort is issued into an **idle** transmitter and the continuous-source premise fails outright there; §4.3 gaining `P' ≥ 1` **guaranteed** and the tail's FCS depending on §6.2's `Preamble` **CRC re-seed**; and fact 8 **kept as written** with the architect's stronger reading recorded and declined, because the observable that would catch an early acceptance is the start character fact 7(a) already asserts); **three** findings of my own (`WO-0083-1` §17.1's plumbing boundary now stated **once as a clause over the whole allow-list** with its test and attached explicitly to items 4 and 5 — *inherited verbatim* is what propagated the asymmetry; `WO-0083-2` `M-21` re-phrased as an **expression** bar the way `M-17` already was, its Revision-A pass standing; `WO-0083-3` §20 item 3's head SHA **quoted at a stated length**, with a first-7 mismatch a hard STOP and a beyond-7 mismatch report-and-proceed); and **one BLOCKED** — my countersignature on SPEC-M04 §6.2's `Preamble` row (*"keeps `tx_tready` = 1"*, unconditionally) against §7's C-16 consequence 4, **re-measured unfixed at this head** (`docs/specs/modules/xgmii_tx_64.md:300`; no §13 change-log row), not mine to fix (`docs/**` is architect_docs_lead's), reachability still **nil** because §4.4's own rule forbids the stimulus that would reach it, carried in the architect's document (`J-architect_docs_lead-0055` Open-question 1). **CELLS THAT MOVED IN THIS PLAN — two, and no more**: **§7 `T-3`** gains **one stated limit of a discharge that is NOT withdrawn** (item (k)) — the `ST-2` schedule-fidelity check compares two quantities computed from the **same expression in the same branch** of the driver, so it cannot fire against that driver: it is a **regression tripwire**, not the live check its own text claims, and the stronger **count-and-position** form (withheld-cycle count equals `hold`, first withheld cycle is the cycle the cursor reached the target) is named with its executor, being a bench edit with a CI-executed result rather than anything a packet revision can land; and **§9's `WO-0083` absorption row above** has its clause *"the wrong design being one octet from conformant"* **struck in place, its text not rewritten** — a change-log row is a dated record of what a round claimed, so the strike marks the claim false and leaves the record readable. **That site, and six more in `WO-0083` itself** (§1's REQ-id line, §2's `M04-F6` index row, §4.2 fact 5's citation of the struck cell, §6.6's `U26` assertion 3, §14's trap `T13`, §15's class-`D2` disposition), **were found by applying the rule `J-dv_lead-0199` banked — enumerate the sites of a struck claim by the CLAIM, never by the string that expressed it — and only three of the seven quote the figure `16` that both earlier sweeps searched for.** Two of the seven repay reading: `T13`'s **trap survives** (a *bench* measuring from the `/E/` really would be one octet out) with only the design-side claim beside it struck; and §15's class-`D2` disposition comes back **stronger** — a `M04-F6` reading of **16** convicts the instrument or the bench and **never** the design, because every gap this decoder can record after an abort is `≡ 7 (mod 8)`, so the readings a design defect can produce are **7** and **23**. **COUNTS, re-measured at this tree by a status-cell pass over every row table after every edit, never carried forward: 83 rows (83 distinct ids), 59 ASSERT, 12 NO-ASSERT, 6 NO-STIMULUS, 5 STRUCTURAL, 1 GAP, 0 RULING** — **no row is added, converted, struck or discharged this round**, discharged stays **41 of 83** and outstanding **42 of 83**, and §6.1's homing census re-run with its own quoted command returns **`83 83 []`**, unchanged and **re-run rather than quoted** (§0.1(i)). **What does NOT move**: no Status cell (this round discharges nothing); no `SO-xgmii_tx_64.md` opened or offered; **`BAR T1` stays SHUT**; REQ-206 is **not** covered and nothing here describes it as covered; `M04-G4` is **still** outstanding and has now missed three consecutive M04 rounds; `DVC-1a` is **still** unbuilt and every M04 count in this round is again a hand count with its method stated; `T-2` is still unbuilt and §4.3's tail-frame is still the live instance that argues for it; and **the `class-11` re-run owed before `P1-module-ready` is untouched by this round** — a packet revision is not a disposition check | dv_lead, `J-dv_lead-0200` |
| 2026-08-22 | **The `RV-0083` item (g) countersignature is recorded — the round that edits no row of this plan and whose whole subject is a SPECIFICATION diff.** architect_docs_lead repaired SPEC-M04's §6.2-`Preamble`-row-versus-§7-C-16-consequence-4 contradiction at **`18de537`** (`J-architect_docs_lead-0064`) in **C-14.2's** shape — the row gains the exception, names the covering §7 clause and says §7 wins; §7's consequence 4 gains one precedence sentence; §13 gains a dated row carrying the provenance chain, including this seat's re-measurement at `J-dv_lead-0200` and naming the countersignature owed. **My verdict is `COUNTERSIGNED`** (`J-dv_lead-0201`, `WO-0083` §21.6), on four grounds: the end convicted is the end my own re-measurement convicted (§7's value is **compelled** — word 0 and word 1 both accepted and untransmitted leave §6.1's two-word depth no register for a third acceptance, so REQ-207 forbids asserting `tx_tready` there — while nothing compelled the row's unconditional form); the shape is the precedent this seat already countersigned once; **no conformant design changes and no committed test changes meaning**; and the §13 row states its provenance without overstating it. **Three ground-survival checks executed at the source** (`git show 18de537 -- docs/specs/modules/xgmii_tx_64.md`, plus the three edited regions at HEAD — row at line **300**, consequence at **481–492**, §13 row at **667**): **(i)** `WO-0083` §4.2 fact 1's ground 2 **survives** — the carved exception's premise names a **predecessor's** post-`tlast` acceptance, which has no instance at a run's **first** frame, so the row pins `tx_tready` = 1 at `C + 1` by its own text and `M04-G5`/`U22`'s red stays dispositive; **(ii)** §4.4's broader ground **survives untightened** — the diff **appends** to consequence 4 and both branches, the first branch's `0` and the second's *"may"*, are byte-identical; **(iii)** the **direction** matches the re-measurement, the row repaired and §7 unmoved. **THE MEASUREMENT BEHIND (i)'s "no committed test changes meaning", stated because it is this plan's kind of fact**: **no unit in `test/**` asserts any value of `tx_tready` at any cycle** — every occurrence of the identifier in the suite sits in a docstring or comment, three of which say so in terms — so nothing landed moves in either direction on this diff **or on the joint item below**. **ONE ITEM IS QUEUED AND IT HAS TWO CONSUMERS IN THIS PLAN — `JOINT-M04-1`** (`WO-0083` §21.6(g.4)), the architect's own new finding, **routed jointly and deliberately not landed**: whether consequence 4's *"may be 1"* is in truth a **pin**. **I refute the derivation offered** — there is no two-register minimum here, and the ≥ 2-cycle accept-to-transmit distance is false as a general statement on §7's own instances (first branch: `C + 8` → `C + 13`, distance **5**; `C + 11` → `C + 14`, distance **3**), with a conformant **late-accept** counter-model that holds `tx_tready` = 0 on the preamble cycle, accepts word 1 the next cycle and emits a byte-identical wire — **and I concur a pin is plausible on a different ground I supply**: that counter-model gives word 0 octets **L = 16** and word 1 octets **L = 8**, two values of L in one frame, which §7's *"L = 16 for every frame octet, of every frame, at every length"* and §0.5's single-valuedness forbid; since the frame's words are contiguous on the wire, the only schedule holding L at one value accepts on consecutive cycles whose second member **is** the preamble cycle. **The open question is therefore about L's DOMAIN**, not about registers: §10's REQ-210 hook measures *"into an idle transmitter after the gap has elapsed"*, which squarely contains a run's first frame **and the frame after an abort**, and doubtfully contains consequence 4's second branch (word 0 accepted at `C + 11`, *"inside the gap"* on §7's own words). **THE TWO CONSUMERS HERE, NAMED NOW SO THE CONSEQUENCE IS NOT DISCOVERED LATER**: `M04-H6` asserts only the first branch's `0` at `C + 12` and routes the second branch away in terms (*"that half is M04-H5's"*), and `M04-H5` is **NO-ASSERT** on the ground that the value is unconstrained — **if the pin lands, H5's routing is wrong at this one cycle and H6 gains a second assertable half.** A third consumer is outside this plan and is the strongest reason to take the question: `test/xgmii_tx_64/bench.ml`'s **P-ACCEPT** precondition (`Bench.present`) requires the accepted cycles to be exactly `C … C + W − 1` **contiguous**, which *is* an acceptance at the preamble cycle, enforced by a `failwith` at disposition class **D3** on every frame it drives — **today design-dependent and uncompelled by any clause; under the L ground, spec-compelled.** **Carrier: the architect's shape-(i) round** (`J-architect_docs_lead-0064` Open-question 1, already declared *"a round with dv_lead in it"*), orchestrator to schedule, architect to own any `docs/**` diff, this seat to supply the derivation and countersign; **if the pin lands the follow-on is mine** — `WO-0083` §4.4's lower bound at the tail frame, H5/H6 here, and P-ACCEPT's grounding. **COUNTS, re-measured at this tree by a status-cell pass over every row table after this edit and NOT carried forward: 83 rows (83 distinct ids), 59 ASSERT, 12 NO-ASSERT, 6 NO-STIMULUS, 5 STRUCTURAL, 1 GAP, 0 RULING** — **no row added, converted, struck or discharged; no Status cell moved; no cell of any row touched; no coverage-map line changed**; discharged stays **41 of 83**, outstanding **42 of 83**; §6.1's homing census re-run with its own quoted command returns **`83 83 []`**, re-run rather than quoted (§0.1(i)). **What does NOT move**: no `SO-xgmii_tx_64.md` opened or offered; **`BAR T1` stays SHUT**; REQ-206 is **not** covered and nothing here describes it as covered; `M04-G4` is **still** outstanding and has now missed four consecutive M04 rounds; `DVC-1a` is **still** unbuilt, so every count in this row is again a hand count with its method stated; `T-2` is still unbuilt; and the `class-11` re-run owed before `P1-module-ready` is untouched — a countersignature is not a disposition check | dv_lead, `J-dv_lead-0201` |
