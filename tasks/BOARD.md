...
|---|---|---|---|
| [BUG-0001](../agents/handoffs/BUG-0001_m03-final-word-over-delivery.md) | dv_lead → rtl_lead | OPEN · CRITICAL | **The programme's first hardware bug**: M03 silently emits max(0, k−4) extra octets whenever a frame's final output word carries k>4 delivered octets (ten of ten tested D fit, 1..1514; tuser=0 and zero strobes at every failing entry). Found by the WO-0038 bench's sixteen-entry table at run 30774152441; prediction P-1 locked in the packet before the probe. Root cause owed by rtl_lead |
