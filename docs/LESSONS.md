...
**What this file is.** The programme's harvested lessons, landed locally at
gate cadence under the harvest law (`agents/PROTOCOL.md` §7 as amended by
[ADR-0023](adr/ADR-0023-the-retraction-mirrored-lessons-are-local.md)):
every module sign-off and phase gate mines the seats' own journals under the
LH1/LH2-g/LH2-d/LH3 bars, the orchestrator collates, and what generalises
lands here — general (`LC-`) candidates in the seat sections, domain (`LD-`)
candidates in domain-titled sections with the grade noted, war stories and
project-tier statements staying in the journals and packets that hold them.
A nil-landing harvest is a normal harvest and is declared in its gate block.

**And the travel copy.** Nothing in this file transmits anywhere by
machinery — the outbound pipeline this programme once built was retracted by
the sponsor on 2026-08-17 (ADR-0023, mirroring the generic shell's own
federation retraction). Learning leaves this repository exactly one way: a
human hands this file, or its tail, to another session or project, and the
receiving end records that as seed provenance. **Baseline: none inherited.**
This programme's corpus begins with its own first harvest (H1, below); its
early experience seeded the generic shell's starter corpus, not the reverse.

**Reading contract.** Each entry is read with provenance hidden first: a
general entry must teach a stranger to both the domain and the project; a
domain entry a stranger to the project who knows the named domain. Citations
resolve in <https://github.com/renatom11/agentic-fpga>. Dedup on landing
(this programme's ADR-0018 as amended): land, merge-by-citation, or drop with
the recurrence recorded; a contradiction is escalated, never silently
resolved; no statement is ever edited in place (A2-D8).

---

## Landing H1 — the first harvest (SO-xgmii_rx_64), landed locally under ADR-0023

- **Parent record**: `SO-xgmii_rx_64` — PASS, fourteen of fourteen, at
  sign-off SHA `41fead6` (`J-dv_lead-0168`, landed `69f1475`), after two
  honest FAILs preserved quoted beneath it.
- **Seeded verbatim from the frozen export packet**
  `docs/federation/outbox/SO-xgmii_rx_64.md` (committed 7fb2c99) — the record
  of the one delivery this programme ever performed under the since-retracted
  transit law (`agents/handoffs/HT-01_first-harvest-transit.md`, EXECUTED;
  the shell-side PR was closed unlanded at the retraction). Every candidate
  section below is byte-identical to the packet's; only this front matter is
  new. The collation method, extraction fidelity checks (14-of-14 sample),
  merge sweeps and hide test are HT-01's §2–§3.
- **Census**: 353 candidates from seven minting chains — dv_lead 95 (94
  `LC-` + 1 `LD-`), architect_docs_lead 94, auditor 54, rtl_lead 50,
  tb_writer 28, orchestrator 18, data_wrangler 14. Ids are seat-qualified
  and unrenumbered (A2 §4.3, grandfathered); they are this file's landed
  local ids.
- **Tier 2**: one row — `LD-SO-xgmii_rx_64-1` (dv_lead), carried inside its
  minting seat's section in the packet's own form, marked **domain tier, pack
  `version-control`, not part of the general body**, its grade named on the
  entry. A future landing with further domain rows opens a domain-titled
  section; this one row stays where its frozen form put it.
- **Recorded annotations carried from HT-01**: four rows were pre-judged
  merge partners of the shell's seeded corpus (`LC-SO-xgmii_rx_64-ADL-45` ↔
  shell `L-B15`, `LC-rtl_lead-H1-8` ↔ `L-D03`, `LC-SO-xgmii_rx_64-AUD-14` ↔
  `L-C15`, `LC-SO-xgmii_rx_64-AUD-10` ↔ `L-E06`) — kept here as annotations
  on their rows; whether the shell ever adopts anything from this file is
  its maintainer's separate act. Two rows (`LC-orchestrator-H1-2`, `-8`)
  carry version-control tool nouns under a stated LH2-g — `OBSERVATION
  SO-O1`, reasoning at HT-01 §3.4, grades the miners' as minted.
- **War stories**: the packet's appendix (47 refused candidates, each with
  the criterion it failed) is part of this landing's record — war stories
  are corpus, kept re-offerable, and bind nowhere.
- **Hide test at landing**: 353/353 clean on project nouns (HT-01 §3.4).
