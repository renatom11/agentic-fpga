...
| 2026-08-03 | **C-40's residual site**: §10's REQ-007/REQ-013 hook still scoped the copy to "emitted **on or after** the input `tlast`", the same phrase the row above corrected at §3 — and the row above called §3 "the sole unpinned instance of a phrase §10 pins", which was **false of §10 itself**. Corrected to "after", pinned to §6.1's D = 0, with the two-activation history named in the cell so a reader is not left to reconcile the two rows | no | none — **editorial**, the last of C-40's six sites; the hook's commissioned assertions (a D = 0 datagram with the bit set, a D ≥ 1 datagram with 0) are unchanged, because they were already written against D and not against the loose phrase. The miss is recorded rather than tidied away: a sweep that names its own completeness is checkable, and this one was checked and was incomplete | `J-architect_docs_lead-0011` |
