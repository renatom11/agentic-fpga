...
| `Preamble` | a first source word is accepted; or, if that word was already accepted on §7's early-acceptance cycle before the gap was served, the cycle after `Idle` is entered (carry-forward **C-16**) | emits the REQ-201 word; seeds the CRC register to 0x00000000; keeps `tx_tready` = 1, **except on the cycle §7's C-16 consequence 4 covers** — where this frame's word 1 has already been accepted (word 0 on the predecessor's post-`tlast` cycle, word 1 on the gap's last cycle), both storage slots hold accepted untransmitted words, `tx_tready` is 0 on this cycle and §7 wins (carry-forward **C-16**) | `Frame`, always, after exactly one cycle |
...
- **Throughput.** One source word accepted per cycle while `tx_tready` is 1;
  ...
  **The cycle after the frame's `tlast` word is accepted — C+8 in §6.1's table —
  ...
  4. **The two cycles cannot both fill both slots.** If a word was accepted at
     ...
     Either way M04 holds at most two accepted, untransmitted words. **This
     consequence wins over §6.2's `Preamble` row**, which keeps `tx_tready` = 1
     on every other cycle of that state: the C-16 diff amended the `Idle` row
     and left the `Preamble` row unqualified, and the row carried the
     unconditional value until §13's 2026-08-22 row. Nothing of this
     consequence's two branches moves in that repair.
...
| 2026-08-22 | **§6.2's `Preamble` row is brought into agreement with §7's C-16 consequence 4, which it has contradicted since the C-16 diff of 2026-08-02 (this table's fourth row).** The row said the state *"keeps `tx_tready` = 1"* with no qualification and named one entry condition (*"a first source word is accepted"*), while C-16 consequence 4 says `tx_tready` is **0** on the preamble cycle of a back-to-back frame whose word 0 was accepted on the predecessor's post-`tlast` cycle and whose word 1 was accepted on the gap's last cycle — and §6.2's own `Idle` row has carried that frame's second entry path since the same diff. **§7 is the winner and the row is the defect**, because the value §7 pins is *compelled*: with word 0 and word 1 both accepted and untransmitted, §6.1's two-word depth (*"not an elastic buffer"*) leaves no register for a third acceptance, so REQ-207 forbids asserting `tx_tready` there — whereas nothing compelled the row's unconditional form, which is the C-16 diff having amended the `Idle` row and left this row as it was, **C-14.2's defect class exactly** (a §6.2 row stating a value §7 later qualified). The repair takes the same shape C-14.2's did: the row carries the exception, names the covering §7 clause and says §7 wins, and gains the second entry condition in the `Idle` row's own words; §7's consequence 4 gains one sentence naming the row as subordinate, and **neither of its two branches moves a character**. **Provenance**: raised by this seat as `J-architect_docs_lead-0055` Open-question 1 and filed there as a carry-forward rather than a bounce (reachability nil for `WO-0083`, whose §4.4 `word ≥ 2` rule excludes the only stimulus that touches it); routed as **`RV-0083` item (g)**; **re-measured unfixed** by dv_lead at `J-dv_lead-0200` / `WO-0083` §21.2 at head `6f165bd` — row still at line 300, consequence unchanged, **no §13 row** — where the countersignature is recorded **BLOCKED** for want of a diff to countersign. **dv_lead's countersignature IS owed on this diff and is not given here**; item (g) closes on a `J-dv_lead-NNNN` entry against this row | no — no port, `Interface` record, state, strobe, gap figure, latency constant or cycle of §6.1's table moves, and **no conformant design changes**: §7 already governed the cycle, REQ-207 with §6.1's two-word depth already forbade the acceptance the unconditional row invited, and §6.1's table depicts the other branch (its C+12 is asserted because no word was accepted at C+8 there). What changes is that the two sections can no longer be read to disagree. **The grounds dv_lead cited this round survive intact**: C-16 consequence 1's uniqueness claim is untouched; consequence 4's second branch (*"may be 1"*, a permission) is untouched; and the row still pins `tx_tready` = 1 at the preamble cycle of a run's **first** frame, which is the only case `WO-0083` §4.2 fact 1's ground 2 uses it for (`J-dv_lead-0200`, item (a): the qualification bites only where an early acceptance has happened, and at the first frame none has) | none — §7 already governed and the row was incomplete, the ground the C-14.1, C-14.2 and C-16 rows above were landed on. SPEC-TEMPLATE rule 7's ADR requirement bites where a post-freeze §6/§7 diff **chooses** between designs; this one records no choice, the losing text being unsatisfiable against REQ-207 rather than rejected. The one live alternative is **not** taken and stays open: `J-architect_docs_lead-0055` Open-question 1's wider question — whether §6.2's rows should carry `tx_tready` values at all now that §7 pins them in four places — is a second, wider diff that would move the `Idle` row dv countersigned at C-14.2 and delete a clause dv is presently citing, so it is left for a round of its own with dv in it | `J-architect_docs_lead-0064` |
