...
| `ip_hdr_valid` | out | 1 | one cycle high per **accepted** datagram, **not later than** one cycle before its first payload word — exactly one cycle before it on a datagram whose input words 2 and 3 are delivered on consecutive cycles, and 1 + k under k idle cycles injected between those two input words (§7); the other six fields are that datagram's | REQ-606 |
...
payload word **on this stimulus**, which satisfies REQ-606's "on or before" with
one cycle to spare and gives M17 a whole cycle to act on the record — the same
lead SPEC-M06 §7 gives M08 and SPEC-M08 §7 preserves for this module.

**What is normative is the bound, and the figure is a gapless one like every
other cycle above** (`FINDING AP-M14-1`, dv_lead, 2026-08-11, §13). The pulse and
payload word 0 have **different deciding input words** — word 2 and word 3 (§7's
per-output-event table) — so the two do not move together under REQ-016's
permitted injection: k idle cycles between those two input words leave the pulse
at C₂ + 1 and put payload word 0 at C₃ + 1 = C₂ + k + 2, a lead of **1 + k**. The
**lower bound survives every such stimulus**, and that is what M17 is written
against: input word 3 is never presented earlier than the cycle after input word
2 (a stream carries at most one word per cycle, REQ-002), so C₃ ≥ C₂ + 1 and
payload word 0 is never earlier than C₂ + 2, while the pulse is at C₂ + 1 — the
record is therefore presented at least one whole cycle **before** the first
payload word on every stimulus, with equality exactly when the two input words
are consecutive. It is presented on the pulse cycle and on no other (§6.3 item
2), which is why a consumer latches it rather than reading it beside the payload. **M17 (batch F) inherits the bound and not the figure**: a
consumer that latches the record on the pulse is correct under both, and a
consumer written to "payload word 0 is the cycle after the pulse" would be
written against a figure this module does not promise.
...
- **Parse latency (REQ-611)**, which is a different measurement on the same
  ...
  before payload word 0 (§6.1). Both figures in that subtraction are gapless
  ones, so the lead it derives is gapless too; the lead's general form is the
  **bound** stated in the handshake bullet below.
...
- **Handshake rules.** `ip_payload_tvalid` = 1 exactly on cycles carrying payload
  ...
  `ip_hdr_valid` is high for **exactly one cycle per accepted datagram**, **not
  later than one cycle before** that datagram's first payload word, and the six
  field values are valid only on that cycle (§6.3 item 2). **The lead is exactly
  one cycle on a datagram whose input words 2 and 3 are delivered on consecutive
  cycles, and 1 + k under k idle cycles injected between those two input words**
  — the pulse's deciding input word is word 2 and payload word 0's is word 3 (the
  table below), so REQ-016's injection at that one site separates two events that
  a gapless stimulus keeps adjacent. **The bound holds on every stimulus REQ-016
  permits**, because input word 3 is never presented earlier than the cycle after
  input word 2 and each of the two events trails its own deciding word by one
  cycle; REQ-606 asks for the record *on or before* the first payload word, so a
  grown lead satisfies it **a fortiori** and gives the consumer more time, never
  less. Scoping the figure while pinning the bound is what this section already
  does for its 3-cycle parse latency under carry-forward **C-27**
  (`FINDING AP-M14-1`, dv_lead, §13). For a datagram of total length 20 it
...
- one `hdr_valid` pulse per frame, exactly one cycle before that frame's first
  payload word (SPEC-M06 §7, preserved by SPEC-M08 §6.1) — **exactly one here
  because this stimulus is gapless inside a frame**, the next bullet putting all
  six words of a frame on consecutive cycles. The producer specifications still
  state that lead **unscoped**, which is a named site of `FINDING AP-M14-1` and
  not repaired here (§13);
...
occupied, and, **on a gapless stimulus**, one cycle before the first payload word
would have left (§6.1's cycle table; under REQ-016 injection the pin is input
word 2 + 1 cycle, §7's table, and only the payload counterfactual moves). **Every
...
| REQ-606 | six fields in a record whose `valid` is one cycle high, **not later than** one cycle before the first payload word — exactly one cycle before it on a datagram whose input words 2 and 3 are consecutive, 1 + k under k idles injected between them (§7, `FINDING AP-M14-1`) | §6.1, §7 | known-datagram test comparing every field and the `valid` timing **on a gapless datagram**; under REQ-016 injection at that site the observables are the bound and the 1 + k figure, never the bare adjacency — a wrapper asserting the adjacency there fails a conformant M14 |
...
| 2026-08-11 | **`FINDING AP-M14-1` SUSTAINED — §7 stated the `ip_hdr_valid` adjacency unscoped and normative while §7's own per-output-event table contradicted it at one injection site, and the adjacency is restated as a BOUND.** The conflict, dv's and verified rather than accepted: the handshake bullet read *"high for exactly one cycle per accepted datagram, on the cycle before that datagram's first payload word"*, §6.1 called that lead *"normative here and not incidental"* because M17 is written against it, and the same section's table puts the pulse at input word **2** + 1 cycle and payload word 0 at input word **3** + 1 cycle — so k idle cycles injected between input words 2 and 3, which REQ-016 permits, make the lead **1 + k** and the two statements cannot both hold. **Ruled with dv's recommendation in substance and one addition**: the adjacency is scoped exactly as §7 already scopes its 3-cycle parse latency under **C-27** (exact on consecutive input words 2 and 3, growing by the injected count), **and the lower bound is stated and derived** rather than left to be re-derived at each reading — input word 3 is never presented earlier than the cycle after input word 2 and each event trails its own deciding word by one cycle, so the lead is **never less than one cycle on any stimulus REQ-016 permits**, with equality exactly on the gapless case. Stating the bound is what keeps §6.1's *"normative"* claim true instead of deleting it, and it is the form REQ-606 is written in (*"on or before the cycle carrying the first payload word"*), which a grown lead satisfies **a fortiori**. **What M17 inherits, since §6.1 named it**: the bound, not the figure. A consumer that latches the six fields on the pulse is correct under every lead ≥ 1 and gains time when the lead grows; a consumer written to *"payload word 0 is the cycle after the pulse"* would be written against a figure this specification does not promise, and SPEC-M17 does not do that — its own §6.1 anchors Ci at *"the first cycle after the opening `ip_hdr_valid` pulse **with `ip_payload_tvalid` = 1**"*, which is robust under a grown lead, so **no diff to SPEC-M17 is owed by this ruling** and none is made. **The alternative that was live and is refused**: hold the adjacency unscoped and make the design defer the pulse to track payload word 0. That would move `ip_hdr_valid`'s deciding input word from 2 to 3, contradicting §7's table and requirements.md §0.5; it would require holding the record for an unbounded idle count; and it would break §9, where the six header-decided strobes of a **rejected** datagram pulse on *"the cycle `ip_hdr_valid` would have occupied"* — a rejected datagram has no payload word to track, so the two would part company. The finding's own arithmetic is preserved above rather than paraphrased away. **Sites, and the list is two longer than the finding's**: §4.2's port row, §6.1's derivation paragraph, §7's handshake bullet, §10's REQ-606 hook and §8's stress bullet were named at `AP-ip_eth_rx_64` §8 item 3; a grep of this file for the claim rather than for the signal name adds **§7's parse-latency bullet** (*"one cycle less than the payload's word delay — which is what puts `ip_hdr_valid` one cycle before payload word 0"*) and **§9's counterfactual** (*"one cycle before the first payload word would have left"*), both repaired here. §10's REQ-016 hook additionally gains the bare adjacency to its list of things a wrapper may not assert under injection, which is what `AP-ip_eth_rx_64` row `M14-F1` was left unable to state. **§12's countersignature row is left as written**: it records what dv re-derived at the freeze, and that derivation was gapless and correct. **Owed and named, not smuggled** — the sibling sites are listed from a grep of every specification for the claim, not estimated: the same unscoped adjacency stands at **SPEC-M06 §6.1, §7 and §10** (M06's §4.2 row states no lead and needs nothing), at **SPEC-M17 §4.2, §6.1, §7, §9 and §10**, with §8's stimulus paragraphs gapless-by-construction as here, and is restated by the wrappers **SPEC-M16 §4.2 and §7** and **SPEC-M19 §4.2 and §7** — each needs its own derivation (different h, different deciding words, and at M17 an interaction with requirements.md §13's 2026-08-04 row, which declared M17's loose REQ-016 sentence *"true and not to be repaired"*), so they are named here and carried rather than swept into a diff written for this module — the discipline that row itself used | no — **editorial, and not behavioural**: no conformant design changes, because a design meeting §7's table already meets the bound and no design could meet the unscoped adjacency under injection; no record, port, width, strobe name or pinned cycle moves; and **no committed stimulus reaches the site** (§8's stress run gaps between frames and delivers each datagram's words on consecutive cycles), so no landed bench changes verdict. What changes is what a bench may assert under injection at one site. **dv_lead's countersignature is NOT owed**: dv is the filer, offered this cure by name at `AP-ip_eth_rx_64` §8 item 3, and its landed row `M14-F1` asserts neither lead and is unmoved either way — the same carve-out dv applied to `AP-M04-2` at `J-dv_lead-0170` §(a). A **contest** remains open to dv on the one point where this ruling exceeds the recommendation: the bound's normativity | none — the retired sentence is unsatisfiable under a stimulus REQ-016 already permits rather than one design chosen over another, and the requirement it serves (REQ-606) is unchanged and unmoved; requirements.md §0.5's deciding-input-word rule supplies the ground | `J-architect_docs_lead-0040` |
