...
| 15 | M07 output word 0; M04 accepts | SPEC-M07 §7's **event delay** — 1 cycle to output word 0, which carries no octet of the frame; *not* its per-octet L, which §7 pins separately |
...
| 2026-08-11 | **§6.1's REQ-502 cycle table, row 15: the citation names which of SPEC-M07 §7's two constants it is composing.** The `From` cell read *"SPEC-M07 §7"* when that bullet pinned one figure; since 2026-08-11 it pins two — an **event delay** of 1 cycle to output word 0, and a per-octet latency L = 22 with q = 6 and ΔC = 2 counted to output word 1 (requirements.md §13's `C-RL-8` row). Row 15's event is *M07 output word 0*, so the constant it composes is the **event delay**, and the cell now says so. **The cycle is unchanged and so is every figure in this table**: cycles 0–16, the seven-cycle result, the residue rule and the `M04-*` rows that quote them are all untouched, and a bench built from this table observes exactly what it observed before. Carried as architect ledger item 59 — *two specifications compose M07's and M15's §7 figures under a name those bullets no longer use exclusively* — of which this is one half; the other, SPEC-M16 §7, took a larger repair in the same round because its composition also carried a false reading of the octets | no — **editorial**: a citation gains the name of the quantity it was already composing correctly | none — a naming-currency repair following requirements.md §13's 2026-08-11 `C-RL-8` row | `J-architect_docs_lead-0042` |
