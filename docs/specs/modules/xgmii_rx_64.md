...
2. **The straddle at a lane-4 start, and the `tlast` word by residue.** *(The
   heading previously read "**Every** output word, at a lane-4 start"; that
   universal is what F-1 below convicts.)* Frame octets 0–3 lie in lanes 4–7 of
   the word after the start word and octets 4–7 in the next (the paragraph
   above), so an output word carrying **more than four** octets is assembled from
   two input words — h = 12 is not a multiple of 8, which is §0.5's straddle
   test — and the wrapper's first legal site falls
   between them. For a **non-`tlast`** word, octets 8m … 8m+3 and octets
   8m+4 … 8m+7 arrive one injected gap apart while D(m), the word carrying octet
   8m + 12, is one gap later still; the word leaves whole because REQ-011 gives no
   encoding for half a word, and its octets therefore measure **L + 16k and L + 8k
   in one word** — **28 and 20 at k = 1**, the two halves of a single output word.
   No conformant design can do otherwise; equalising them would require splitting
   the word.

   **The `tlast` word does not follow that split, and at seven of the eight
   residues it is not what this item used to claim** (dv_lead, **F-1** against
   `J-architect_docs_lead-0025`). Its D is the closing character's word — call it
   T — and not an octet four beyond its own, so item 1's rule governs it: an octet
   measures **L + 8k′** for the k′ idles injected between its own input word and
   D. Under a uniform wrapper at k that is derivation 1's residue table read at
   lane 4, and nothing else:

   | r = N mod 8 | the `tlast` word's delivered octets | where they lie | they measure |
   |---|---|---|---|
   | 0 | bytes 0–3 (four octets) | the word before T | **L + 8k** = 20 at k = 1 |
   | 1, 2, 3 | bytes 0–3, then bytes 4 … r+3 | bytes 0–3 in the word before T; bytes 4 … r+3 in **T itself** | **L + 8k** and **L** = 20 and **12** |
   | 4 | bytes 0–7 (a full word) | bytes 0–3 two words before T, bytes 4–7 one | **L + 16k** and **L + 8k** = 28 and 20 — the split above |
   | 5, 6, 7 | bytes 0 … r−5 (r − 4 octets) | the word before T | **L + 8k** = 20 at k = 1 |

   Only r = 4 puts a **full** `tlast` word two input words behind its own closing
   character, so only there is the split of the preceding paragraph the `tlast`
   word's own arithmetic; at the other seven residues that word is short, and
   reading the split onto it overstates its octets by 8k. The r ∈ {1, 2, 3} row is
   derivation 1's set read at lane 4: the closing character's word is later than
   the word carrying the last delivered octet at r ∈ {0, 4, 5, 6, 7}, so its
   complement {1, 2, 3} is exactly where the last delivered octets **share** the
   closing character's word and are separated from their evidence by no injected
   idle at all — the mirror of the lane-0 r ∈ {5, 6, 7} case item 3 states, which
   is why both items state it or neither is right. The table holds at every length
   delivering at least one octet, single-output-word frames included.

   **What that costs a reader, in classes.** A frame of at least two output words
   measures **three** values at r ∈ {1, 2, 3} — `{L, L + 8k, L + 16k}`, so
   `{12, 20, 28}` at k = 1 and `{12, 68, 124}` at k = 7 — and **two**,
   `{L + 8k, L + 16k}`, at the other five residues, the `tlast` word contributing
   no value the non-`tlast` words do not already carry there. §8's directed set
   puts the three-class case at lane-4 lengths **65, 66 and 67**, which §10's
   REQ-016 hook drives at k = 1 and k = 7, so this is printed output on the next
   run and not a corner: a bench built from this item's previous wording would see
   three reported values where it predicted two and read a conformant design as
   divergent. As everywhere in this item the values are **reported** and SHALL NOT
   be asserted (§0.5, item 3).

   *Provenance of the two corrections this item has now taken. The figures 20 and
   12 it first carried were this split read against the withdrawn D(m), and were
   re-based to 28 and 20 at `J-architect_docs_lead-0025`; that ruling left the
   **scope** at "every output word", which is false for the `tlast` word at every
   residue but 4 — the cell dv_lead refused as numerically incomplete (**F-1**,
   `J-dv_lead-0086`), verified independently and repaired at
   `J-architect_docs_lead-0026`. The arithmetic above is F-1's own in every cell;
   only the wording is mine, F-1's offered "the split otherwise" being itself
   inexact at r = 0 and r ∈ {5, 6, 7}, where the `tlast` word measures L + 8k and
   not the split.*
...
**A note for a future consumer of an internal closure record — explicitly
non-normative, and not a DV observable.** *Nothing in this paragraph constrains a
conformant design, and no bench may assert any of it.* §6.3 item 2 leaves the
...
record inside it. A normative sentence here would take that carve-out back and
commission a test the specification cannot support, which is dv_lead's stated
condition for the note living here at all (`J-dv_lead-0089`, §7 of
`RV-0060-VERDICT`); it is guidance in the sense REQ-019's payload-storage
sentence already uses, so `traceability.md` claims no coverage for it. It is
recorded because the trap was found by building against it, and a trap
rediscovered is a defect. **The trap.** An implementation that realises the
closure list above as a *record* — the closing event captured when its character
is decoded — will generally hold that record **one cycle ahead of the octets it
decides**: clause (a) evaluates the event at its own octet time, so the record is
born at age 0 on the closing character's own input word, while that word's octets
reach the emission decision one cycle later through the alignment stage. Gapless
the skew is invisible, because on the record's own cycle the word its closure
ends has not yet reached the emission register; insert any elastic hold in the
path — REQ-016's injected idle is enough — and the two separate. A consumer
comparing such a record against the **aligned** octet stream must therefore read
it through an alignment-corrected view rather than through the raw record-valid.
In the landed M03 that view is `closure_aligned` (`J-rtl_lead-0010`): at a lane-0
start (alignment offset 0) the aligned view spans the previous input word, so the
record is admitted only at **age ≥ 1**; at a lane-4 start (offset 4) the aligned
view already contains lanes 0…3 of the cycle's own word, so an **age-0** record
is admitted there. A consumer that reads the raw record instead releases the
closing word **one cycle early**, and that is a measured quantity rather than a
prediction: on the 64-octet lane-0 member the `tlast` word is emitted at cycle
**19** at k = 1 and **67** at k = 7 — the cycles §6.1's D(m) puts it at, the
closing character's word being that word's decider — where the
alignment-uncorrected release gives **18** and **66** (`J-dv_lead-0089`, Evidence
6). **The one-cycle figure is this decode placement's, not this
specification's**: an implementation that decodes closure at a different point in
its pipeline has a different skew or none at all, which is exactly why §6.3
item 2 stands unamended and this paragraph asserts nothing.
...
| REQ-802, REQ-810 | `cfg_rx_enable` sampled at the start character and gating **admission only**; when 0 no frame is accepted, so nothing is emitted and no strobe pulses **for a frame that would have begun**, while a frame already in flight completes under the old value including under REQ-110 (§4.3, ADR-0014); the same-cycle change is unconstrained (§6.3 item 7) | §4.3, §6.1, §6.2, §6.3 | drive `cfg_rx_enable` = 0 and inject 100 frames; assert no word and no strobe, then re-enable. The change SHALL be driven at least one cycle away from any start character — a same-cycle change has no determinate outcome and SHALL NOT be asserted on (C-14.5). **Then the mid-frame case**: open a frame with `cfg_rx_enable` = 1, drop it to 0 at least one cycle before a start character that arrives while the frame is still open, and assert the in-flight frame is aborted at the octet before it with `tuser`[0] = 1 on its `tlast` word, **exactly one** `error_start_without_terminate`, **no** output word for the frame that start character would have begun, and the next frame received normally after the enable returns to 1 (**M03-N4**). **The zero-delivered branch of that case has no instance at M03.** Until 2026-08-09 this hook carried it as a parenthetical — *"(or no output word where it had delivered none)"* — and it is quoted here rather than deleted, because the branch was once commissioned and a reader who does not find the record re-derives it from REQ-110 and asks again (dv_lead, **M03-N4**). **The ground is this specification's own arithmetic and not a bench limitation.** The aborted frame is *admitted*, so `cfg_rx_enable` = 1 on its own start cycle; it delivers no octet only where the aborting start character lies at or before its first octet (§9's ninth row), which §6.1's eight-octet preamble and §0.3's lane-0-or-lane-4 rule confine to that frame's **own start word or the word immediately after it**; and the 1 → 0 change must fall strictly after the admitting start cycle and at least one cycle before the aborting one, so the admissible interval is **empty, at both start lanes**. Neither escape opens it: moving the start character moves both words together (§0.3), and inserting an idle between a start character and the frame's first octet is what the REQ-016 hook above forbids in terms (**M03-N3**). **Stated so that no sign-off packet claims coverage of a zero-delivered REQ-110 abort here.** That geometry *is* commissioned, under `cfg_rx_enable` = 1, at the REQ-110 hook above; this hook's delivering branch is untouched by the withdrawal and stays commissioned whole |
...
| 2026-08-04 | **FINDING F-1 RULED — VERIFIED and repaired: §6.1 item 2's lane-4 split is the **non-`tlast`** words' arithmetic, and the `tlast` word now carries its own residue table.** dv_lead countersigned the `1f3c04c` D(m) ruling three points in full and **REFUSED the lane-4 class cell** as numerically incomplete (**F-1**, the COUNTERSIGNATURE block of `agents/handoffs/BUG-0002_m03-idle-injection-tlast-on-a-non-final-word.md` at `a8ca14d`, `J-dv_lead-0086`; requirements.md §13's transcription row records it outstanding). Re-derived here from §0.5's octet-time definition and this section's own geometry rather than from the finding: at a lane-4 start an octet of output word m measures L + 8k′ for the k′ wrapper boundaries between its own input word and D(m), and the `tlast` word's D is the closing character's word T, which sits **one** word behind its octets at r ∈ {0, 1, 2, 3, 5, 6, 7} and two only at r = 4. Hence bytes 0–3 measure **L + 8k** and not L + 16k at seven of the eight residues, and at r ∈ {1, 2, 3} the further bytes 4 … r+3 lie in **T itself** and measure **L**, giving a **third** class `{12, 12+8k, 12+16k}` — `{12, 20, 28}` at k = 1, `{12, 68, 124}` at k = 7 — at directed lengths **65, 66 and 67**, inside §8's own set and §10's REQ-016 hook. Every cell of dv_lead's table reproduces exactly; **its offered wording does not and is declined** — "the split otherwise" is refuted by F-1's own r = 0 and r ∈ {5, 6, 7} sentences, so the numbers are adopted and the phrasing derived fresh. The repair is one paragraph plus a four-row table inside item 2 | no — **nothing moves but a description**: D(m) is untouched, no cycle this specification pins moves at any k, no requirement, port, record, `Interface` field or REQ hook changes, and no conformant design changes. The values corrected were never assertable — §0.5 and item 3 bar demanding a per-octet L under injection and M03-I4 reports rather than asserts them — so **no committed test changes meaning and nothing goes red or green by this row**; what changes is what a reader, or a bench built from this item, would predict on printed output | none — **the defect does not reach requirements.md §0.5** and that is a derivation, not a convenience: §0.5's straddle and late-decision bullets, its "no surviving residue class" verdict on M03 and its monitor prohibition are each true as written under this repair, and §0.5 delegates this detail in terms ("*SPEC-M03 §6.1 states the residues*"), so the false sentence was homed here and is repaired here. No design alternative existed to record: item 2 was wrong about arithmetic, not about a choice. **Countersignature class — the refusal *is* the review, and no new dv_lead signature is owed**: this row repairs a cell dv_lead itself refused, to values dv_lead itself derived and published in a committed, signed artefact, so a further countersignature would be dv_lead signing its own derivation back to itself. That standing holds because the repair (i) departs from F-1's numbers in no cell, (ii) moves no rule, cycle or requirement, and (iii) does not reach normative programme-wide text; a repair failing any of the three would owe the signature under the `J-architect_docs_lead-0025` row's own condition. Dispute of any cell of the new table is a **fresh finding**, not an unsigned condition on this row — it is in force on commit. dv_lead is owed **notification**, not signature: F-1 is discharged and its `AP-xgmii_rx_64.md` M03-I4 repair may state the three-class case | `J-architect_docs_lead-0026` |
| 2026-08-04 | **The closure-record age skew recorded as a trap — §9 gains one explicitly non-normative note and nothing else.** rtl_lead's `J-rtl_lead-0010` escalation 3, routed here by dv_lead's `RV-0060-VERDICT` §7. The fact, established in the landed implementation (`fafb83d`) and measured at word 7: an implementation that realises §9's closure list as a *record* holds it one cycle ahead of the octets it decides — clause (a) evaluates the closing event at its own octet time, so the record is born at age 0 on the closing character's own input word while that word's octets reach the emission decision a cycle later through the alignment stage. Gapless the skew is invisible; under REQ-016's injected idle it separates, and a consumer reading the raw record-valid rather than an alignment-corrected view (`closure_aligned`: age ≥ 1 at offset 0, age 0 admitted at offset 4) releases the closing word **one cycle early** — the 64-octet lane-0 word-7 discriminator, **19/67 against 18/66** (`J-dv_lead-0089`, Evidence 6). The note names the one-cycle figure as *this decode placement's*, not the specification's | no | none — **the lightest class in this log, and lighter than an editorial row**: an editorial row repairs text that was being read normatively, where this row adds text that is normative nowhere. It states no SHALL, moves no sentence, pins no cycle, adds and amends no requirement or REQ hook, changes no `Interface` field, commissions no test and closes no ledger item; **§6.3 item 2 stands unamended**, which is the point — a *normative* §9 sentence about how a consumer must read an internal record would retract that carve-out and commission a test whose object (the age of an internal record) is not this module's observable. **dv_lead's condition, binding under its testability-countersigner role, is the form of the note and is met in terms**: marked non-normative and not a DV observable, in the manner REQ-019's payload-storage sentence already uses, so `traceability.md` claims no coverage and **no countersignature is owed** (`RV-0060-VERDICT` §7: "*as guidance it costs nothing and owes me no countersignature*"). Were it ever restated normatively that would be a fresh, narrowly countersigned change | `J-architect_docs_lead-0027` |
| 2026-08-09 | **§10's REQ-802/REQ-810 hook: the mid-frame case's parenthesised zero-delivered branch declared to have NO INSTANCE at M03, in the form §10's REQ-014 hook already uses.** dv_lead's `WO-0069` item 1, derived while authoring the bench packet for the row this hook commissions (`WO-0068` §5, `J-dv_lead-0121`) and landed plan-side at `J-dv_lead-0124` before it was raised here. **Re-derived from this specification's own text rather than accepted**: the aborted frame is admitted, so the enable is 1 on its start cycle; zero delivered puts the aborting `/S/` at or before that frame's first octet (§9's ninth row), which §6.1's eight-octet preamble and §0.3's lane-0-or-lane-4 rule confine to the frame's own start word `S` or `S+1`; the 1 → 0 change must land in `[s+1, w−1]`, empty for `w ∈ {s, s+1}` — at **both** start lanes, with §6.3 item 7 (**C-14.5**) forbidding the one cycle (`s` itself) that would otherwise open it. Both escapes closed on normative text: §0.3 confines start characters to lanes 0 and 4, so enlarging the front offset moves `S` and `W` together; and §6.1 and §10's REQ-016 hook forbid the injection wrapper from placing an idle between a start character and the first octet (**M03-N3**). The parenthetical is **quoted inside its own withdrawal, not deleted**, on dv_lead's ground — deletion loses the record that the branch was once commissioned | no | none — **editorial, and the class is §10's own REQ-014 row above**: a verification column commissioning an observable that has no stimulus at this module. **No normative sentence moves** — §4.3, §6.1, §6.2, §6.3 item 7, §9 rows 8 and 9, REQ-110, REQ-802 and REQ-810 are each individually right, are unchanged, and are what the derivation is *taken from*; **ADR-0014 is untouched in every clause** and is not reopened. SPEC-TEMPLATE rule 7 puts a post-freeze change to §4, §6 or §7 behind an ADR and leaves everything else needing "*only a journal entry*", so **this diff is unilateral and owes neither an ADR nor a countersignature** — the precedent is this table's 2026-08-03 REQ-014 row and requirements.md §13's C-46 and C-39/C-41 verification-column rows, and the C-43 row is the line it stays on the right side of (normative text, not a verification column, is what carries the countersignature discipline). What changes is what a sign-off packet may **claim**; the hook's delivering branch, which is landed and green at `2dbd39b` in CI `build` run **`30988038809`**, is untouched, and **no row of `AP-xgmii_rx_64.md` moves in either direction** — dv_lead's own bound on this item, and the plan's M03-N4 cell already carries the finding against itself | `J-architect_docs_lead-0031` |
