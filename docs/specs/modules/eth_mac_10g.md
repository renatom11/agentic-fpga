...
| 11.2 | **REQ-017's port-list check runs at M20, not here**, so M05's four wire-side names are checked only by the compile lane until batch F. | **CLOSED (WO-0019).** SPEC-M20 §4.1 declares `xgmii_rx` and `xgmii_tx` from the same `Xgmii` record with the same two `rtlprefix` values this module uses, and SPEC-M20 §10's REQ-017 hook commissions the port-list check against `rtl_snapshots/nic_top` — the check `tools/check_emitted_verilog.sh` already carries as a PENDING row until M20 is emitted. The names were fixed by the type all along; the M20 check confirms them and this row was always a pointer to where. | SPEC-M20 §4.1, §10 | architect_docs_lead | closed |
