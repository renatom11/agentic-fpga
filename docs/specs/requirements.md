...
**Cycles.** A latency constant is converted to cycles as floor(L / 8). REQ-006's
end-to-end budget and REQ-019's per-module ceilings are stated in cycles and are
compared against that figure.
...
| **REQ-006** | PERF | **Receive latency budget.** The delay from the XGMII word containing the start character of a frame to the first word of that frame's UDP payload on the application receive stream SHALL be at most 24 cycles (153.6 ns). | Cycle-tagged end-to-end bench at `nic_top`. Measurement events: the cycle of the XGMII word carrying the start character, and the cycle of the first application `tvalid` word of the same frame. The value is measured and reported separately for a lane-0 and a lane-4 start (they may legitimately differ by one cycle, §0.5); both are recorded in the module spec freeze record and in the Phase-1 latency report. |
...
| **REQ-010** | IFC | **Typed stream fabric.** All frame-carrying ports SHALL use the program stream types `Axi64.Source` and `Axi64.Dest` (`Hardcaml_axi.Stream.Make` with `data_bits` = 64, `user_bits` = 1). Ad-hoc per-module stream records are prohibited. | Interface compile check of every module spec's `I` and `O` records against the pinned toolchain, including a type-identity witness per stream port — `let _check (x : Signal.t Axi64.Source.t) = x` applied to the record field — which fails to compile for a hand-written record with the same fields. The RTL-side binding is rtl_lead's obligation, evidenced by each module's `.mli`. |
...
| **REQ-019** | INV | **Bounded receive latency, no deep buffering.** Each receive-path module's pinned latency constant, converted to cycles per §0.5, SHALL be no greater than its ceiling in §1.1. No receive-path module SHALL contain payload storage deeper than two datapath words; header fields captured into registers are not payload storage and the ARP cache is not on the payload path — that second sentence is design guidance and is explicitly **not** a DV observable, so `traceability.md` claims no test coverage for it. | The pinned constant in each receive-path module spec is checked against §1.1 at spec freeze, and the measured per-octet latency from the REQ-004 stress run (REQ-005) is checked against the same ceiling in each sign-off packet. |
...
24-cycle budget. Each module spec pins its exact constant; these are the
ceilings the pinned constants must fit inside, compared as floor(L / 8) per
§0.5. Slack is released only by a spec diff, so an over-budget module is a
visible decision rather than an accumulation.
...
| **REQ-903** | PROC | **Module surface.** Every module in the inventory SHALL have an `.mli` and a `hierarchical` entry point taking a `Scope.t`. | Mechanical repository check at each module-ready gate. |
